...
public struct EnrolmentResult {
    ...
    // Genesis TSK seed — passed to recovery arming ONLY, then not retained. It is NEVER stored on
    // the identity tree (post-genesis it lives only in the 2-of-3 recovery shares); see the Python
    // reference (build_identity_tree takes it as input; enrol_recovery takes it as a separate arg).
    public let tskSeed: Data
}
...
public final class EnrollmentCeremony {
    ...
    public func enrol(signalSource: SignalSource,
        ...
        return EnrolmentResult(identity: identity, enrollmentSecret: enrollmentSecret,
                               forensicWindowOpen: forensicWindow, tskSeed: tskSeed)
    }
