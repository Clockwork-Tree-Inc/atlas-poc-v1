...
final class GroupRelay {
    ...
    // #43: rotate MESSAGE mailboxes per (pair channel key, epoch) so the relay can't build the
    // social graph from stable "to" addresses. Discovery/handshake (register, roster, KEM, contrib)
    // stays on the stable mailbox so setup can never break; ONLY MSG delivery rotates. Toggle
    // `atlas.relay.rotating` = false to fall back to stable delivery. (Sealed-sender — hiding the
    // "from" — is the complementary step; here we rotate the recipient address.)
    // Rotation ON: message mailboxes rotate per (pair key, epoch). Requires a node that auto-creates
    // mailboxes on first use (node_server relay_send/fetch auto-vivify) — deploy your node with that
    // before testing, or rotating sends are rejected. Flip atlas.relay.rotating=false to fall back
    // to stable mailboxes.
    private let rotating = (UserDefaults.standard.object(forKey: "atlas.relay.rotating") as? Bool) ?? true
    private static let epochSeconds = 30.0
    private static let epochWindow: UInt64 = 4          // ±4 epochs catch-up (~2 min of clock skew / downtime)
    private func epochRound() -> UInt64 { UInt64(max(0, Date().timeIntervalSince1970 / GroupRelay.epochSeconds)) }
    ...
    func start() {
        ...
        loop = Task { [weak self] in
            ...
            // Register with RETRY until it succeeds — a transient failure (e.g. the node briefly
            // unreachable) must self-heal, not leave us polling an unregistered mailbox forever.
            var registered = false
            while !Task.isCancelled && !registered {
                do { try await self.register(); registered = true; self.report("registered at node as \(self.me)") }
                catch {
                    self.report("connecting… (\(Self.explain(error)))")
    ...
    private func tick() async {
        ...
        // #43: also drain the rotating MESSAGE mailboxes for each established pair, scanning a
        // catch-up window of epochs so downtime / clock skew never drops a message.
        if rotating {
            for p in Array(channels.keys) {
                guard let ck = channels[p] else { continue }
                for epoch in Mailbox.catchUpEpochs(nowRound: epochRound(), window: GroupRelay.epochWindow) {
                    guard let mbox = try? Mailbox.derive(pairSecret: ck, sender: Data(p.utf8),
                                                         recipient: Data(me.utf8), epoch: epoch) else { continue }
                    if let msgs = try? await fetchInbox(mailbox: hex(mbox)) {
                        for (frm, blob) in msgs { await handle(frm: frm, blob: blob) }
                    }
                }
            }
        }
    ...
    func send(_ text: String) {
        ...
        let rot = rotating, epoch = Mailbox.epochForRound(epochRound())
        Task {
            guard let sealed = try? Tunnel.sealNormalBlob(Data(text.utf8), key: msgKey(lk)) else { return }
            for p in peers {
                // #43: address this epoch's ROTATING mailbox for the pair (derived from the channel
                // key), so the relay sees a fresh unlinkable "to" each epoch instead of a stable name.
                if rot, let ck = channels[p],
                   let mbox = try? Mailbox.derive(pairSecret: ck, sender: Data(me.utf8),
                                                  recipient: Data(p.utf8), epoch: epoch) {
                    try? await relay(to: hex(mbox), blob: GroupRelay.msgP + sealed, from: "")   // sealed sender
                } else {
                    try? await relay(to: p, blob: GroupRelay.msgP + sealed, from: "")            // stable fallback, sealed sender
                }
            }
        }
    }
    ...
    // transport
    private func relay(to peer: String, blob: Data, from: String? = nil) async throws {
        // Handshake blobs carry `from: me` (the peer must know who to answer). MESSAGE blobs pass
        // from: "" — SEALED SENDER: the real sender name already rides INSIDE the encrypted payload
        // ({n,t,a}), so the relay never learns who sent, only an opaque rotating "to".
        _ = try await post("relay/send", ["from": from ?? me, "to": peer, "blob": blob.base64EncodedString()])
    }
    private func fetchInbox(mailbox: String? = nil) async throws -> [(String, Data)] {
        let data = try await get("relay/fetch/\(mailbox ?? me)")
        guard let o = try JSONSerialization.jsonObject(with: data) as? [String: Any],
