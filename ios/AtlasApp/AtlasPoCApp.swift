...
struct AtlasPoCApp: App {
    ...
    init() { DevLog.shared.installCrashHandlers() }
    ...
    var body: some Scene {
        WindowGroup {
            ContentView()
                ...
                .onAppear {
                    PushManager.shared.session = session
                    PushManager.shared.start()   // request permission + register for silent wakes
                }
                .onChange(of: session.currentPersona?.handle) { _, _ in
                    PushManager.shared.reregister()   // re-bind token to the active persona's mailbox
                }
        }
    }
}
