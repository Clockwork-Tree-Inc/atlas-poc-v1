...
extension RingProbe: @preconcurrency CBPeripheralDelegate {
    ...
    /// Wall-clock time of the last detected live pulse. Lets a timer-driven caller detect
    /// removal by staleness (`now - lastLivePulseTime`), which works even when the ring goes
    /// silent on removal and `pulsePresent` freezes (it only updates on frame receipt).
    var lastLivePulseTime: TimeInterval { lastHRTime }

    /// Recent ring dynamic-motion magnitude series over the last `windowS` seconds, for
    /// same-body correlation against the phone IMU. Empty if the R10 isn't streaming accel
    /// (its accel stream is intermittent — the same limit that gates the tap-bind).
    func ringMotionSeries(windowS: Double) -> [(t: TimeInterval, mag: Double)] {
        let now = Date().timeIntervalSince1970
        return accelSamples.filter { now - $0.t <= windowS }
    }
...
extension RingProbe: Wearable {
    ...
    var capabilities: WearableCapabilities {
        ...
        return caps                                          // never .secureElement (passive ring)
    }
