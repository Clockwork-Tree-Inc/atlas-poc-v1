...
public enum AtlasFlags {
    ...
    // MARK: - User-facing disclaimer (shown by PoCBanner / DisclaimerView)

    /// Persistent one-line strip shown on every screen.
    public static let poCBadge = "PROOF OF CONCEPT — work in progress, not for real use"

    /// Short disclaimer for the opening / enrolment screen.
    public static let disclaimerShort =
    ...
    /// Full plain-language disclaimer for the Disclaimer tab / sheet. The
    /// `honestyBanner` list below breaks down what is real vs stood-in vs stubbed.
