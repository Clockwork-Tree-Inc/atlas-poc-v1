...
final class RecoveryTiersTests: XCTestCase {
    ...
    func testDefaultPolicyReachesSocialWithoutCard() {
        let f = Factors(guardianQuorum: true, namePassword: true, recoveryCard: false)
        XCTAssertTrue(RecoveryTiers.reachableTiers(f).contains(.social))                            // default
        XCTAssertTrue(RecoveryTiers.reachableTiers(f, RecoveryTiers.RecoveryPolicy()).contains(.social))
    }

    func testPolicyCardRequiredBlocksSocialWithoutCard() throws {
        let policy = RecoveryTiers.RecoveryPolicy(requireCardForSocial: true)
        let f = Factors(guardianQuorum: true, namePassword: true, recoveryPerson: true, recoveryCard: false)
        XCTAssertFalse(RecoveryTiers.reachableTiers(f, policy).contains(.social))
        XCTAssertEqual(try RecoveryTiers.selectTier(f, policy), .physicalSelf)
    }

    func testPolicyCardRequiredSocialReachableWithCard() throws {
        let policy = RecoveryTiers.RecoveryPolicy(requireCardForSocial: true)
        let f = Factors(guardianQuorum: true, namePassword: true, recoveryCard: true)
        XCTAssertTrue(RecoveryTiers.reachableTiers(f, policy).contains(.social))
        XCTAssertEqual(try RecoveryTiers.selectTier(f, policy), .social)
    }

    func testPolicyNeverAffectsTheFloor() throws {
        let policy = RecoveryTiers.RecoveryPolicy(requireCardForSocial: true)
        let f = Factors(namePassword: true, recoveryPerson: true, recoveryCard: false)
        XCTAssertEqual(try RecoveryTiers.selectTier(f, policy), .physicalSelf)
        XCTAssertTrue(RecoveryTiers.neverLockedOut(f))
    }
}
