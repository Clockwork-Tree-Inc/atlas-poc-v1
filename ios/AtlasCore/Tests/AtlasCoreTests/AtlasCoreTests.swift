...
final class AtlasCoreTests: XCTestCase {
    ...
    func testRatchetForwardSecrecy() {
        ...
        let k1 = Derivation.ratchet(k0, entropyT: entropy, beaconT: Data("b".utf8), epochRound: Data(count: 8))
        // Attacker with k0 but not the secret entropy cannot reach k1.
        let guess = Derivation.ratchet(k0, entropyT: Data(count: 32), beaconT: Data("b".utf8), epochRound: Data(count: 8))
        XCTAssertNotEqual(guess, k1)
        XCTAssertEqual(Derivation.ratchet(k0, entropyT: entropy, beaconT: Data("b".utf8), epochRound: Data(count: 8)), k1)
    }
    ...
    func testStratifiedRecovery() throws {
        ...
        let enr = try Recovery.enrol(tree, tskSeed: seed, biometricTemplate: bio, device: device, passcode: "hunter2", sphincs: sphincs)
        ...
        XCTAssertEqual(try Recovery.recoverViaCard(enr, device: device, cardShare: enr.shareCard,
            liveBiometric: bio, attested: true, userAuthorized: true, sphincs: sphincs).tskPublic, tree.tskPublic)
        XCTAssertEqual(try Recovery.recoverInPerson(enr, device: device, liveBiometric: bio,
            attested: true, inPersonTrustedContext: true, userAuthorized: true, sphincs: sphincs).tskPublic, tree.tskPublic)

        // Enclave's robust matcher accepts a 25%-noisy casual read (device-present)
        let casual = noisy(bio, frac: 0.25)
        XCTAssertEqual(try Recovery.recoverViaCard(enr, device: device, cardShare: enr.shareCard,
            liveBiometric: casual, attested: true, userAuthorized: true, sphincs: sphincs).tskPublic, tree.tskPublic)
    ...
    func testLivenessGateLiveVsSpoof() {
        ...
        XCTAssertTrue(gl.state(sensorDigest: Data("d".utf8), epochRound: Data(count: 8)).operate)
        let gs = LivenessGate(); for (_, l) in Synthetic.spoofStream(40) { gs.update(pSGivenLive: l.0, pSGivenNotLive: l.1) }
        XCTAssertFalse(gs.state(sensorDigest: Data("d".utf8), epochRound: Data(count: 8)).operate)
    }
    ...
    func testRecognitionSharedTunnelAndTwoModes() throws {
        ...
        let lk = Data(repeating: 1, count: 32), epochKey = Data(repeating: 2, count: 32), epochRound = Data(count: 8)
        try A.advanceEpochPresent(lk: lk, epochKey: epochKey, epochRound: epochRound)
        try B.advanceEpochPresent(lk: lk, epochKey: epochKey, epochRound: epochRound)
        ...
        let pole = gate.state(sensorDigest: Data("d".utf8), epochRound: epochRound)
