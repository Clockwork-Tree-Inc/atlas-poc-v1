...
final class AuthTests: XCTestCase {
    ...
    private func livePole() -> PoLEState {
        ...
        return g.state(sensorDigest: Data("s".utf8), epochRound: Data(count: 8))
    }
    private func deadPole() -> PoLEState {
        PoLEState(pLive: 0, stateDigest: Data("d".utf8), epochRound: Data(count: 8), operate: false)
    }
    ...
    private func persona(_ username: String = "alice") throws -> Profile {
        try IdentityTree.build(tskSeed: Primitives.randomBytes(32), sphincs: StubSphincs()).profile(username)
    }

    func testPerRPAuthenticatorUnlinkableAcrossRPs() throws {
        let p = try persona()
        let a = try rpAuthenticator(p, "acme-bank")
        let b = try rpAuthenticator(p, "gov-portal")
        XCTAssertNotEqual(a.publicKey.encode(), b.publicKey.encode())     // distinct key per RP
        XCTAssertNotEqual(a.handle, b.handle)                             // distinct handle per RP
        XCTAssertEqual(try rpAuthenticator(p, "acme-bank").handle, a.handle)  // deterministic per (persona, RP)
        XCTAssertNotEqual(try rpAuthenticator(try persona("bob"), "acme-bank").handle, a.handle)  // cross-persona unlinkable
    }

    func testAuthenticateToRPUsesPerRPKey() throws {
        let p = try persona()
        let authnA = try rpAuthenticator(p, "acme-bank")
        let ch = chal("acme-bank")
        let a = try authenticateToRP(ch, profile: p, pole: livePole())
        XCTAssertTrue(verifyAssertion(a, ch, registeredHandle: authnA.handle, registeredPublic: authnA.publicKey))
        XCTAssertNotEqual(try rpAuthenticator(p, "other-bank").publicKey.encode(), authnA.publicKey.encode())
    }
}
