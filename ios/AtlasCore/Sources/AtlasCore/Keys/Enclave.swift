...
public protocol BiometricEnclave {
    ...
    /// A SILENT, hardware-bound contribution for the per-tick DEVICE-CLOCK session key —
    /// folded into the RAM-only session key each tick so it can't be reproduced off THIS
    /// device's Secure Enclave. Device-LOCAL (non-deterministic on real HW), so it is never
    /// part of any cross-device or KAT'd derivation. On real HW the underlying SE anchor key
    /// rotates on the device clock (10 ± 2 s).
    func hardwareTickContribution(_ input: Data) -> Data
}
...
public final class ModelEnclave: BiometricEnclave {
    ...
    /// Deterministic software stand-in for tests/parity — the real SE binding + 10±2s anchor
    /// rotation live in `SecureEnclaveStore` on device.
    public func hardwareTickContribution(_ input: Data) -> Data {
        Primitives.hkdf(ikm: master, info: Data("atlas/se/tick|".utf8) + input, length: 32)
    }
}
