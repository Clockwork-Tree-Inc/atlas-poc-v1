...
public enum Params {
    ...
    // M7 hardening (mirrors backend/atlas/params.py): clamp each per-sample likelihood
    // into [eps, 1-eps] so no single crafted sample dominates; require a minimum number
    // of samples before the gate may operate. Below every legitimate stream's minimum.
    public static let livenessLikClampEps = 0.01
    public static let livenessMinSamples = 5
