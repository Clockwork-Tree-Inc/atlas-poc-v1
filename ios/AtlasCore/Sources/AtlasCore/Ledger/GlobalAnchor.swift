...
public final class GlobalAnchorLog {
    ...
    @discardableResult
    public func anchor(ownerID: Data, root: Data, epochRound: Data) throws -> GlobalReceipt {
        let round = epochRound.reduce(UInt64(0)) { ($0 << 8) | UInt64($1) }  // big-endian -> UInt64
        ...
        let entryHash = Primitives.H(Self.label, prev, Self.lp(ownerID), Self.lp(root),
                                     Self.lp(epochRound), withUnsafeBytes(of: &ib) { Data($0) })
    ...
    public func verifyChain() -> Bool {
        ...
        for (i, e) in entries.enumerated() {
            ...
            let expect = Primitives.H(Self.label, prev, Self.lp(e.ownerID), Self.lp(e.anchoredRoot),
                                      Self.lp(e.epochRound), withUnsafeBytes(of: &ib) { Data($0) })
            ...
            let round = e.epochRound.reduce(UInt64(0)) { ($0 << 8) | UInt64($1) }
