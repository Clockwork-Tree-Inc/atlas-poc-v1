...
/// The PER-RELYING-PARTY authenticator key for a persona. Each relying party gets a
/// DISTINCT keypair — hence a distinct handle AND public key — derived under the persona
/// seed (`feature("rp:" + name)`), so two relying parties cannot correlate the same person
/// by a shared key/handle, and two personas at one RP are likewise unlinkable. Mirrors the
/// Python reference `rp_authenticator`.
public func rpAuthenticator(_ profile: Profile, _ relyingParty: String) throws -> Child {
    try profile.feature("rp:" + relyingParty)
}

/// Safe-by-construction: derive the PER-RP authenticator for THIS challenge's relying party,
/// so a caller cannot accidentally present one shared key across relying parties (which would
/// make the person linkable across them).
public func authenticateToRP(_ challenge: AuthChallenge, profile: Profile, pole: PoLEState,
                             yubikey: YubiKeyBio? = nil, fingerprintMatched: Bool = false) throws -> VerifiedHumanAssertion {
    let authn = try rpAuthenticator(profile, challenge.relyingParty)
    return try authenticate(challenge, authorship: authn, pole: pole,
                            yubikey: yubikey, fingerprintMatched: fingerprintMatched)
}
