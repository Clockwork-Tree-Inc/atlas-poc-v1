...
///
/// A `BeaconRound` is an external-drand round — Atlas's BOOTSTRAP / defence-in-depth
/// anchor, NOT the epoch key (that is `EpochBeacon`/`EpochRound`). Before the
/// aggregator's epoch beacon has fired, this round's index doubles as the epoch
/// round (bootstrap); thereafter it is used only as an anchor.
public struct BeaconRound: Sendable, Equatable {
    ...
    /// 8-byte big-endian round index, same shape as `EpochRound.epochRound()` so a
    /// bootstrap round is a drop-in epoch-round label.
...
/// External drand — the BOOTSTRAP / defence-in-depth anchor (§3.2), NOT the epoch
/// key. drand on the Mac; deterministic stand-in for offline/preview. Both conform
/// to `Beacon`.
