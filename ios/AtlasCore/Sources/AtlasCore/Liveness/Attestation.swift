...
public struct LivenessAttestation {
    ...
    public init(epochRound: Data, poleDigest: Data, operate: Bool,
                enclavePublic: HybridSign.PublicKey, signature: Data, challenge: Data = Data()) {
        self.epochRound = epochRound
    ...
    /// message is an INJECTIVE function of (epochRound, poleDigest, operate,
    /// challenge). A plain `|`-delimited concatenation is ambiguous — a 0x7c
    /// byte inside epochRound (raw beacon randomness) is an alternative split point,
    /// letting one signature re-parse to a different epochRound/challenge. Mirrors
    /// `hkdfCombine`'s length-prefixing discipline and Python's `message_for`:
    /// `b"".join(len(p).to_bytes(4,"big") + p for p in parts)`.
    static func messageFor(epochRound: Data, poleDigest: Data, operate: Bool, challenge: Data = Data()) -> Data {
        let flag = Data([operate ? 0x01 : 0x00])
        let parts: [Data] = [Data("atlas/attest".utf8), epochRound, poleDigest, flag, challenge]
        var out = Data()
        for p in parts {
    ...
    public func verify() -> Bool {
        HybridSign.verify(enclavePublic,
                          Self.messageFor(epochRound: epochRound, poleDigest: poleDigest, operate: operate, challenge: challenge),
                          signature)
    }
}
...
public final class AttestationSubsystem {
    ...
    public func attest(_ pole: PoLEState, challenge: Data = Data()) -> LivenessAttestation? {
        ...
        let msg = LivenessAttestation.messageFor(epochRound: pole.epochRound, poleDigest: pole.stateDigest,
                                                 operate: true, challenge: challenge)
        let sig = (try? HybridSign.sign(enclaveKey, msg)) ?? Data()
        return LivenessAttestation(epochRound: pole.epochRound, poleDigest: pole.stateDigest, operate: true,
