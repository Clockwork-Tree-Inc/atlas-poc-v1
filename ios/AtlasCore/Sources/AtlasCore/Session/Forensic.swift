...
public struct ForensicChunk: Sendable {
    ...
    public static func computeHash(seq: Int, cause: String, epochRound: Data, randomness: Data,
        ...
        return Primitives.H(Data("atlas/forensic/link".utf8), seqBytes, Data(cause.utf8),
                            epochRound, randomness, prevHash, ciphertext)
    }
}
...
public final class ForensicWindow {
    ...
    public func capture(_ plaintext: Data, beacon: BeaconRound) throws -> ForensicChunk {
        ...
        seq += 1
        let epochRound = beacon.epochRound()
        ...
        let hash = ForensicChunk.computeHash(seq: seq, cause: cause.rawValue, epochRound: epochRound,
                                             randomness: rnd, prevHash: head, ciphertext: ct)
        let chunk = ForensicChunk(seq: seq, cause: cause.rawValue, beaconDrandRound: epochRound,
...
public func openForensicWindow(header: ForensicHeader, chunks: [ForensicChunk],
    ...
    for (i, c) in chunks.enumerated() {
        ...
        let expect = ForensicChunk.computeHash(seq: c.seq, cause: c.cause, epochRound: c.beaconDrandRound,
