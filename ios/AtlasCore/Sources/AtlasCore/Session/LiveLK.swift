...
///
/// INVARIANT: only fresh secret VALUES are combined as key material. The epoch
/// round is used solely as HKDF domain-separation CONTEXT (`info`), never as a
/// combined value — so it binds the LK to its epoch without becoming an input to
/// the secret. External drand is not involved (the epoch round is the aggregator's
/// QRNG epoch index, not a drand round); timing never enters the value either.
/// Combination is order-independent (contributions are sorted), so A and B agree
...
public enum LiveLK {
    ...
    public static func coDeriveLK(_ contributions: [Data], epochRound: Data) throws -> Data {
        guard contributions.count >= 2 else {
            throw LiveLKError.tooFewContributions
        }
        let ordered = contributions.sorted(by: lexLess)
        return Primitives.hkdfCombine(ordered, info: lkInfo + epochRound, length: 32)
    }
