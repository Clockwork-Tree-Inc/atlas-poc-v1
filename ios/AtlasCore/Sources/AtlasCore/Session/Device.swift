...
public struct EpochInputs {
    ...
    public init(lk: Data, epochKey: Data, epochRound: Data) { self.lk = lk; self.epochKey = epochKey; self.epochRound = epochRound }
}
...
public final class Device {
    ...
    public func wrapEpochKey(_ epochKey: Data, epochRound: Data) throws -> Data {
        try Presence.wrapEpochKey(epochKey, enrollmentSecret: enrollmentSecret, epochRound: epochRound)
    }
    ...
    @discardableResult
    public func advanceEpoch(wrappedEpochKey: Data, wrappedLK: Data, epochRound: Data,
        ...
        let epochKey: Data
        do { epochKey = try Presence.unwrapEpochKey(wrappedEpochKey, presenceSecret: secret, epochRound: epochRound) }
        catch { throw PresenceRequired.epochUnwrapFailed }
        let lk: Data
        do { lk = try Presence.unlockLK(wrappedLK, epochKey: epochKey, epochRound: epochRound) }
    ...
    @discardableResult
    public func advanceEpochPresent(lk: Data, epochKey: Data, epochRound: Data,
                                    pole: PoLEState? = nil) throws -> SessionKey {
        let p = pole ?? PoLEState(pLive: 1.0, stateDigest: Primitives.H(Data("atlas/present".utf8), epochRound),
                                  epochRound: epochRound, operate: true)
        let wrappedLK = try Presence.wrapLK(lk, epochKey: epochKey, epochRound: epochRound)
        let wrappedEpochKey = try wrapEpochKey(epochKey, epochRound: epochRound)
        return try advanceEpoch(wrappedEpochKey: wrappedEpochKey, wrappedLK: wrappedLK, epochRound: epochRound,
    ...
    @discardableResult
    public func continuityTick(_ pole: PoLEState, epochRound: Data, beacon: Data,
        ...
        let advanced = continuityKey!.withBytes { ck in
            Derivation.ratchet(Data(ck), entropyT: entropyT, beaconT: beacon, epochRound: epochRound)
        }
    ...
    public func messageRatchetStep(_ prev: Data, beaconT: Data, epochRound: Data) -> (next: Data, entropy: Data) {
        let entropy = Primitives.randomBytes(32)
        return (Derivation.ratchet(prev, entropyT: entropy, beaconT: beaconT, epochRound: epochRound), entropy)
    }
}
