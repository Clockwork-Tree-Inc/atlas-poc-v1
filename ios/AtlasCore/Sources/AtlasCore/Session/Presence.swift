...
public enum Presence {
    ...
    private static func unwrapKey(_ enrollmentSecret: Data, epochRound: Data) -> Data {
        Primitives.hkdf(ikm: enrollmentSecret, info: Data("atlas/epoch-unwrap|".utf8) + epochRound, length: 32)
    }
    ...
    public static func wrapEpochKey(_ epochKey: Data, enrollmentSecret: Data, epochRound: Data) throws -> Data {
        try Primitives.aeadEncrypt(key: unwrapKey(enrollmentSecret, epochRound: epochRound), plaintext: epochKey, aad: unwrapAAD)
    }
    ...
    public static func unwrapEpochKey(_ wrapped: Data, presenceSecret: Data, epochRound: Data) throws -> Data {
        try Primitives.aeadDecrypt(key: unwrapKey(presenceSecret, epochRound: epochRound), blob: wrapped, aad: unwrapAAD)
    }
    ...
    private static func lkKey(_ epochKey: Data, epochRound: Data) -> Data {
        Primitives.hkdf(ikm: epochKey, info: Data("atlas/lk-unlock|".utf8) + epochRound, length: 32)
    }
    ...
    public static func wrapLK(_ lk: Data, epochKey: Data, epochRound: Data) throws -> Data {
        try Primitives.aeadEncrypt(key: lkKey(epochKey, epochRound: epochRound), plaintext: lk, aad: lkAAD)
    }

    /// Unlock the LK with the (unwrapped) epoch key. Throws if the epoch key is
    /// wrong — no epoch key -> no LK.
    public static func unlockLK(_ wrappedLK: Data, epochKey: Data, epochRound: Data) throws -> Data {
        try Primitives.aeadDecrypt(key: lkKey(epochKey, epochRound: epochRound), blob: wrappedLK, aad: lkAAD)
    }
}
