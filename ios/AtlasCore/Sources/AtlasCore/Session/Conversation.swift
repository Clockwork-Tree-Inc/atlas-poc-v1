...
enum ConvCore {
    ...
    static func aad(direction: Data, index: Int, epochRound: Data) -> Data {
        Primitives.H(Data("atlas/conv/aad".utf8), direction, u64(index), epochRound)
    }
    /// The accountable signing core: binds WHO (via the signing key) to WHAT
    /// (content) at WHICH position (index/direction/epoch).
    static func sigCore(content: Data, direction: Data, index: Int, epochRound: Data) -> Data {
        Primitives.H(Data("atlas/conv/sig-core".utf8),
                     Primitives.H(Data("atlas/conv/content".utf8), content),
                     direction, u64(index), epochRound)
    }
...
public struct Envelope: Sendable {
    ...
    public var epochRound: Data
    public var ciphertext: Data
    public var signature: Data

    public init(index: Int, direction: Data, epochRound: Data, ciphertext: Data, signature: Data = Data()) {
        self.index = index; self.direction = direction; self.epochRound = epochRound
        self.ciphertext = ciphertext; self.signature = signature
    }

    public func toWire() -> Data {
        let obj: [String: Any] = [
            "i": index, "d": direction.base64EncodedString(), "e": epochRound.base64EncodedString(),
    ...
    public static func fromWire(_ blob: Data) throws -> Envelope {
        ...
        return Envelope(index: i, direction: d, epochRound: e, ciphertext: c, signature: s)
    }
}
...
final class SendChain {
    ...
    init(ck: Data, epochRound: Data, beaconT: Data, index: Int = 0) {
        self.ck = ck; self.epochRound = epochRound; self.beaconT = beaconT; self.i = index
    }

    func seal(_ plaintext: Data, direction: Data) throws -> (index: Int, ct: Data) {
        let (mk, next) = FSConversation.step(ck, beaconT: beaconT, epochRound: epochRound)
        let index = i
        ck = next; i += 1                              // advance; old chain key discarded
        let ct = try Primitives.aeadEncrypt(key: mk, plaintext: plaintext,
                                            aad: ConvCore.aad(direction: direction, index: index, epochRound: epochRound))
        return (index, ct)
    }

    func snapshot() -> [String: Any] {
        ["ck": ck.base64EncodedString(), "epoch": epochRound.base64EncodedString(),
         "beacon": beaconT.base64EncodedString(), "i": i]
    }
    static func restore(_ s: [String: Any]) -> SendChain {
        SendChain(ck: Data(base64Encoded: s["ck"] as! String)!, epochRound: Data(base64Encoded: s["epoch"] as! String)!,
...
final class RecvChain {
    ...
    init(ck: Data, epochRound: Data, beaconT: Data, next: Int = 0, skipped: [Int: Data] = [:]) {
        self.ck = ck; self.epochRound = epochRound; self.beaconT = beaconT
    ...
    func open(index: Int, ciphertext: Data, direction: Data) throws -> Data {
        ...
        if index < next {
            ...
            while next < index {                       // cache the skipped keys
                let (skmk, skn) = FSConversation.step(ck, beaconT: beaconT, epochRound: epochRound)
                skipped[next] = skmk
                ck = skn; next += 1
            }
            let (thismk, thisn) = FSConversation.step(ck, beaconT: beaconT, epochRound: epochRound)
    ...
    func snapshot() -> [String: Any] {
        ...
        return ["ck": ck.base64EncodedString(), "epoch": epochRound.base64EncodedString(),
    ...
    static func restore(_ s: [String: Any]) -> RecvChain {
        ...
        return RecvChain(ck: Data(base64Encoded: s["ck"] as! String)!, epochRound: Data(base64Encoded: s["epoch"] as! String)!,
...
public final class Conversation {
    ...
    public static func create(mode: ConversationMode, myDirection: Data, peerDirection: Data,
                              channelKey: Data, lk: Data, epochRound: Data, beaconT: Data,
                              authorship: Child? = nil, peerPublic: HybridSign.PublicKey? = nil) throws -> Conversation {
        let sSeed = FSConversation.seedChain(channelKey: channelKey, lk: lk, epochRound: epochRound, direction: myDirection)
        let rSeed = FSConversation.seedChain(channelKey: channelKey, lk: lk, epochRound: epochRound, direction: peerDirection)
        return try Conversation(mode: mode, myDirection: myDirection, peerDirection: peerDirection,
                                send: SendChain(ck: sSeed, epochRound: epochRound, beaconT: beaconT),
                                recv: RecvChain(ck: rSeed, epochRound: epochRound, beaconT: beaconT),
    ...
    public func send(_ plaintext: Data) throws -> Envelope {
        ...
        var sig = Data()
        if mode == .accountable {
            let core = ConvCore.sigCore(content: plaintext, direction: myDir, index: index, epochRound: send.epochRound)
            sig = try HybridSign.sign(authorship!.keypair, core)
        }
        return Envelope(index: index, direction: myDir, epochRound: send.epochRound, ciphertext: ct, signature: sig)
    }
    ...
    public func receive(_ env: Envelope) throws -> Data {
        ...
        if mode == .accountable {
            ...
            let core = ConvCore.sigCore(content: plaintext, direction: env.direction, index: env.index, epochRound: env.epochRound)
