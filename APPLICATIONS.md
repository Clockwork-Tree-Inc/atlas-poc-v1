...
> **Honest boundary:** the verified-human authenticator, the origin/action-bound challenge, relay/replay rejection, the liveness gate, and the live `/verify/presence` service **are built**. The WebAuthn piece (`auth/webauthn.py`) is a **field/format mapping + demo**, *not* a production WebAuthn stack — real CBOR / COSE keys / attestation still need a vetted library (`AuthenticationServices` on device, a WebAuthn server lib on the RP). "Drop-in passwordless passkey login for a live bank" is not yet wired.
