...
This catalog is **exhaustive on purpose** and **honest about maturity**. Overall posture:
**beta — open for testing**, not "finished"; the built core is real and offered for others to run
and attack, and every tag below still means exactly what it says (nothing sim/designed is presented
as proven). Each item is tagged:

| Tag | Meaning |
|---|---|
| ✅ | **Built + tested** — implemented and covered by the CI test suite (788 Python + 313 Swift tests) |
...
- ✅ **Split-TSK identity** — at creation the System-ID splits into a **server half + user half**. The **user half is secure-element-bound** — held only inside the SE of each **enrolled device** (phone / recovery card / ring), never a software value. Recovery is **user-chosen m-of-n custodians**, and **whether the SE recovery card is *required* for guardian recovery is the user's own policy choice** (`RecoveryPolicy.require_card_for_social`, `recovery/tiers.py`): **off (default)** — a guardian quorum + name/password can recover a lost-device user (design of record); **on** — guardian recovery also needs the card, so losing it drops to in-person and card-seizure alone can't water down authenticity. The user bears the loss from loose security, so the user owns the trade-off. Server half non-exportable; **no operator path**.
...
- ✅ **Swappable live-signal source** — one `SignalSource` seam behind ambient (phone sensors) or ring biology; source-agnostic timed ratchet. The **ambient (phone-sensor) source is confirmed functional**: it drives the engine end-to-end on the **security + identity axis** (entropy/timing → key derivation, ratchet, identity/session machinery). This makes **no liveness claim on its own** — anti-spoof is the *separate* signal-quality axis (ambient → ring → high-fidelity wearable). Never read this as "liveness proven via ambient."
...
- 📐 **Minimal mode** — drop to telephony + SOS only on attestation mismatch
...
- ✅ **Records assembly (medical + sensitive)** — four grants (patient-own · treating-clinician episode · retained encounter note · threshold reopen: doctor-share ∧ governing-body-share) · break-glass (loud: logged + notifies) · **retention ≠ readability** (expiry as beacon-round arithmetic) · tamper-evident *symmetric* access log (the `notify` bit bound into the hash chain); a **sensitive-record space tier** (policy roles gate WHO, records grants gate WHEN)
- ✅📱 **In-app records** — create by typing *or* **importing any file (PDF / image / document)**, role-gated open, rendered by kind, live access-log verify
- ✅📱 **Provenanced PDF export** — render a document to a PDF stamped on every page with author · did:cid · content-hash · signature (recompute + verify who authored it, unaltered); part of document creation
- ✅ **C2PA embed service (node)** — bakes the presence VC into content as a signed **C2PA manifest** where the c2pa lib runs; **images live**, PDF pending the binding's `application/pdf` support
- ✅ **Guardian forensic access** — a trusted contact collects your panic black-box only under **mutual consent** (Shamir 2-of-2: they hold a share, you release yours on panic — neither alone opens it)
...
- ✅ **Recovery tiers** — **device-present** (hold an enrolled SE device → unseal directly); **social** (device lost → a user-chosen m-of-n guardian quorum + name/password, **plus the SE recovery card if the user's policy requires it**); **physical-self floor** (all gone / policy-forced → in-person: name+password + a live accountable recovery person + server threshold — never permanently locked out under any policy, the last credential is you). Total-loss stores no biometric; passcode PBKDF2. *(user's-choice policy: `RecoveryPolicy.require_card_for_social` — 9 existing + 4 policy tests green.)*
...
- ✅ **Three entity classes** — individual · organization {nonprofit \| for-profit, sector category} · agent; verification-not-authority (verified = holding valid issuer attestations, no anointed root)
- ✅ **Agent-delegation leash** — an AI agent acts only under a signed, capability-scoped, expiring delegation whose chain must **root at a non-agent** (human/org); attenuating sub-delegation; a single enforcement point cross-checks the root against a real credential (an agent can't self-declare being a human)
- ✅ **Credential issuers** — accountable authorities that **mint + revoke** real-id / registration / sector-category attestations the gates consume
- ✅ **Real-ID supply gate** — earn / sell / own-an-org requires Real-ID (an org also registration); **buying stays anonymous** — the regulated boundary in the right place
- ✅ **did:cid self-certifying identifiers** — content-addressed DIDs (the id *is* the key's hash) for Universal-Resolver interop, no ledger/host
- ✅ **ePassport Real-ID (chip)** — NFC eMRTD read with **passive authentication** (data-integrity + signature) *and* **active/chip authentication** (anti-clone); honest issuer-confirmed-vs-not tiering (ICAO CSCA master list)
- ✅ **Space-policy access substrate** — membership + role ladder (reader/contributor/governor/break-glass) + M-of-N quorum governance + an append-only hash-chained audit log (audit *and* sync); one mechanism for sharing, officer-quorum, and agent capability-scope
- ✅ **Anonymous-eligible polls & reviews** — a verified member of the right space/role polls anonymously: personhood-gated eligible set + per-poll nullifier (one vote, unlinkable across polls) + shown sample size
...
- ✅ **Consumable verified-human service — LIVE** — `POST /verify/presence` returns `{valid, verified_human, assurance_tier, epoch, subject}` for a presented credential, with a **min-tier gate** (AMBIENT vs WEARABLE); `POST /did/register` publishes a persona key under its **did:atlas + did:cid** so an app posts **token-only** (no key passed). *Verified end-to-end against the live public node over HTTPS.*
- ✅ **One presence claim, every standard format** — `presence_interop` emits a Proof-of-Living-Entropy attestation as a **W3C Verifiable Credential**, an **OpenID Connect ID Token** ("Sign in with Atlas"), and the **C2PA-manifest payload** — the same claim, each **verifiable offline by signature** (no phone-home; personas don't correlate across apps)
- ✅ **Drop-in RP SDK + 15-minute quickstart** — dependency-free Python + JS clients (`is_live_human(node, token, min_tier=2)`), `docs/VERIFY_QUICKSTART.md`
...
> **Economics in beta = receipts, not tokens.** During beta, economic activity is tracked only by
> **non-monetary receipts** (participation / activity proofs). The transferable **Atlas PoLE coin /
> token is NOT live and is NOT offered in beta** — it goes live only at **full release**. **No token
> is offered in beta; this is not an investment; the token launches only at full release.** The items
> below are `📐` **designed**, not shipped.
...
- ✅📱 **PoLE receipts wallet** — presence receipts **auto-minted on the clock** (each live tick; the user never signs by hand) committing to fused live entropy, with multi-stream corroboration (persona + wearable co-sign). **Non-monetary** — backs UBI eligibility, not a token.
- ✅ **Paid attention (an Attention-proof)** — a store pays a **proven live human** to look at a product — the inverse of ads — paid **once per human** (per-offer nullifier), so a bot with no live presence can't farm it. Settled in **beta receipts, not tokens.**
...
- **~1478 automated tests in CI** — **1037 Python** across 128 files (incl. dedicated `test_security_properties`, `test_threat_model`, `test_value_timing`, `test_coupled_epoch_gaps`, plus the verified-human interop + records + entity-model + economy suites) + **441 Swift** including byte-exact Python↔Swift **parity vectors**.
