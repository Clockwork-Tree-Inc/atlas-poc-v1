...
def test_send_to_unknown_mailbox_autocreates_for_rotation():
    """A send to a not-yet-registered mailbox AUTO-CREATES it (the box springs into existence on
    delivery). This is required for ROTATING mailboxes (#43) — they are derived, never pre-
    registered — and matches the reference MailboxRelay. The recipient can then fetch it.
    (A production node rate-limits / TTL-expires idle boxes to bound this; out of scope here.)"""
    node = AtlasNode()
    code, out = _call(node, "POST", "/relay/send", {"from": "a", "to": "ghost", "blob": _b64(b"x")})
    assert code == 200 and out["ok"]
    _, got = _call(node, "GET", "/relay/fetch/ghost")
...
def test_optin_public_provenance_verify_and_anchor():
    ...
    pole = g.state(sensor_digest=b"s", epoch_round=rnd.epoch_round())
    reg = PublicWitnessRegistry(); reg.publish(lk, rnd.epoch_round())  # noqa: E702
