...
def test_no_continuity_no_epoch_key_no_lk_no_ratchet():
    ...
    w_lk = wrap_lk(lk, epoch_key=ek, epoch_round=eid)
    w_ek = A.wrap_epoch_key(ek, eid)
    live = PoLEState(p_live=1.0, state_digest=b"d", epoch_round=eid, operate=True)
    dead = PoLEState(p_live=0.0, state_digest=b"d", epoch_round=eid, operate=False)

    def go(dev, *, wep=w_ek, wlk=w_lk, bio=None, pole=live):
        return dev.advance_epoch(wrapped_epoch_key=wep, wrapped_lk=wlk, epoch_round=eid,
    ...
    with pytest.raises(PresenceRequired):
        go(A, wlk=wrap_lk(lk, epoch_key=b"Z" * 32, epoch_round=eid))
...
def test_recognition_yields_shared_tunnel_key_without_sending_session_keys():
    A, B = _pair()
    inp = EpochInputs(lk=b"L" * 32, epoch_key=b"E" * 32, epoch_round=b"\x00" * 8)
    A.advance_epoch_present(lk=inp.lk, epoch_key=inp.epoch_key, epoch_round=inp.epoch_round)
    B.advance_epoch_present(lk=inp.lk, epoch_key=inp.epoch_key, epoch_round=inp.epoch_round)
...
def test_recognition_rekeys_when_beacon_advances():
    A, B = _pair()
    inp = EpochInputs(lk=b"L" * 32, epoch_key=b"E" * 32, epoch_round=b"\x00" * 8)
    A.advance_epoch_present(lk=inp.lk, epoch_key=inp.epoch_key, epoch_round=inp.epoch_round); B.advance_epoch_present(lk=inp.lk, epoch_key=inp.epoch_key, epoch_round=inp.epoch_round)
...
def test_outsider_cannot_compute_recognition():
    ...
    C, D = _pair()
    C.advance_epoch_present(lk=inp.lk, epoch_key=inp.epoch_key, epoch_round=inp.epoch_round); D.advance_epoch_present(lk=inp.lk, epoch_key=inp.epoch_key, epoch_round=inp.epoch_round)
...
def test_mode2_verified_human_only_gate():
    ...
    pole = gate.state(sensor_digest=b"d", epoch_round=inp.epoch_round)
...
def test_stolen_device_cannot_open_mode2_after_wipe():
    ...
    spoof_pole = g.state(sensor_digest=b"d", epoch_round=inp.epoch_round)
...
def test_message_ratchet_forward_secrecy_and_break_in_recovery():
    ...
    A, _ = _pair()
    k0 = os.urandom(32)
    k1, entropy1 = A.message_ratchet_step(k0, beacon_t=b"b1", epoch_round=b"\x00" * 8)
    # Attacker captured k0 but NOT the secret entropy1: cannot reach k1.
    guess = ratchet(k0, entropy_t=b"\x00" * 32, beacon_t=b"b1", epoch_round=b"\x00" * 8)
    assert guess != k1
    # With the real (secret) entropy, the ratchet is reproducible.
    assert ratchet(k0, entropy_t=entropy1, beacon_t=b"b1", epoch_round=b"\x00" * 8) == k1
