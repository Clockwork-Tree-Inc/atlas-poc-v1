...
def test_calm_known_present_is_routine_and_allows():
    d = decide_and_record(_ledger(), decision=DecisionType.LOGIN, epoch_round=b"e0",
...
def test_suspicious_signals_escalate():
    for sig in [
        ...
        d = decide_and_record(_ledger(), decision=DecisionType.LOGIN, epoch_round=b"e",
...
def test_failed_factors_are_denied_regardless_of_risk():
    d = decide_and_record(_ledger(), decision=DecisionType.HIGH_STAKES, epoch_round=b"e",
...
def test_every_decision_leaves_exactly_one_event():
    ...
    for i, sig in enumerate([Signals(), Signals(factors_ok=False),
        ...
        decide_and_record(ledger, decision=DecisionType.LOGIN, epoch_round=f"e{i}".encode(),
                          context_handle=_handle(), signals=sig)
...
def test_forged_event_from_another_device_fails_verify():
    ledger = _ledger()
    decide_and_record(ledger, decision=DecisionType.LOGIN, epoch_round=b"e0",
                      context_handle=_handle(), signals=Signals())
...
def test_ledger_is_opaque_at_rest():
    ledger = _ledger()
    handle = _handle()
    decide_and_record(ledger, decision=DecisionType.PAYMENT, epoch_round=b"e0",
