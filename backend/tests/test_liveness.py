...
def _run(stream):
    ...
    return g.state(sensor_digest=b"digest", epoch_round=b"\x00" * 8)
...
def test_attestation_signed_each_step():
    ...
    assert a.epoch_round == st.epoch_round


def test_attestation_message_is_injective():
    """The signed message must bind exactly one (epoch_round, pole_digest, operate,
    challenge). A `|`-delimited concatenation is ambiguous when a field contains
    0x7c (epoch_round is raw beacon randomness); length-prefixing makes it injective,
    so a genuine signature cannot be re-parsed into a different epoch_round/challenge."""
    from atlas.liveness.attestation import LivenessAttestation as LA
    # sliding the epoch_round/pole_digest boundary must NOT collide
