...
def _device(name="A", clock=None):
    ...
    d.advance_epoch_present(lk=b"L" * 32, epoch_key=b"E" * 32, epoch_round=b"\x00" * 8)
    return d
...
def test_operate_tick_advances_key_and_emits_fresh_attestation():
    d = _device()
    t1 = d.continuity_tick(_pole(live_stream(40)), epoch_round=b"\x00" * 8, beacon=BEACON, challenge=b"chal-1")
    assert t1.operate and t1.attestation is not None
    assert t1.attestation.verify() and t1.attestation.challenge == b"chal-1"
    t2 = d.continuity_tick(_pole(live_stream(40)), epoch_round=b"\x00" * 8, beacon=BEACON, challenge=b"chal-2")
...
def test_realised_interval_does_not_enter_the_key(monkeypatch):
    ...
    def key_for(bio_byte):
        ...
        return d.continuity_tick(_pole(live_stream(40)), epoch_round=b"\x00" * 8, beacon=BEACON).continuity_key
...
def test_stale_or_absent_beacon_is_fail_closed_not_fail_stale():
    ...
    tick = d.continuity_tick(_pole(live_stream(40)), epoch_round=b"\x00" * 8, beacon=b"")
...
def test_liveness_break_tick_wipes_and_advances_no_key():
    ...
    _ = d.session.key
    tick = d.continuity_tick(_pole(spoof_stream(40)), epoch_round=b"\x00" * 8, beacon=BEACON)
