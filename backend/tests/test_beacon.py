...
def test_drand_rejects_missing_signature():
    """3b fix: a relay that OMITS the signature must not skip verification by omission. With
    verification on (default), a signature-less response is a rejection."""
    def stub(url):
        return {"round": 42, "randomness": "aa" * 32}   # no 'signature' field
    with pytest.raises(ValueError):
        DrandHTTPBeacon(http_get=stub).latest()


def test_drand_info_rejects_relay_that_lies_about_genesis():
    """3c fix: genesis/period are pinned constants; a relay that disagrees is REJECTED, not
    adopted (a lie here shifts every timestamp uniformly while every signature still verifies)."""
    from atlas.beacon.drand import QUICKNET_GENESIS_TIME, QUICKNET_PERIOD_S
    def liar(url):
        return {"period": QUICKNET_PERIOD_S, "genesis_time": QUICKNET_GENESIS_TIME + 31_536_000}
    with pytest.raises(ValueError):
        DrandHTTPBeacon(http_get=liar).info()


def test_drand_info_accepts_relay_that_agrees():
    from atlas.beacon.drand import QUICKNET_GENESIS_TIME, QUICKNET_PERIOD_S
    def honest(url):
        return {"period": QUICKNET_PERIOD_S, "genesis_time": QUICKNET_GENESIS_TIME}
    DrandHTTPBeacon(http_get=honest).info()             # no raise
