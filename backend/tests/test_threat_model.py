...
def _pole(stream, att, epoch=b"\x00" * 8):
    ...
    return g.state(sensor_digest=b"s", epoch_round=epoch), att
...
def test_T02_proof_tokens_epoch_bound_and_expiring():
    ...
    att = AttestationSubsystem()
    pole_a, _ = _pole(live_stream(40), att, epoch=rnd_a.epoch_round())
    ledger = LedgerStub()
    registry = PublicWitnessRegistry()
    lk, sk = os.urandom(32), os.urandom(32)
    registry.publish(lk, rnd_b.epoch_round())
...
def test_T08_stolen_card_share_alone_insufficient():
    ...
    enr = R.enrol_recovery(tree, seed, bio, device=device, passcode="pw")
...
def test_T18_epoch_rollover_replay_rejected():
    ...
    def epoch(t):
        ...
        ek = hashlib.sha256(b"atlas/epoch-qrng" + r.epoch_round()).digest()
        A.advance_epoch_present(b"L" * 32, ek, r.epoch_round())
        B.advance_epoch_present(b"L" * 32, ek, r.epoch_round())
        _, tB = establish_hybrid_tunnel(A, B, b"c|" + r.epoch_round())
        return tB
...
def test_T06_proof_token_tamper_rejected_and_no_key_on_wire():
    ...
    a = att.attest(pole)
    assert a.verify()
    tampered = type(a)(epoch_round=a.epoch_round, pole_digest=a.pole_digest, operate=a.operate,
