...
def test_global_anchor_chain_and_lookup():
    ...
    r1 = led.root
    rec1 = g.anchor(b"user-A", r1, epoch_round=(1).to_bytes(8, "big"))
    led.append(commit(b"m1")[0])
    r2 = led.root
    g.anchor(b"user-A", r2, epoch_round=(2).to_bytes(8, "big"))
    g.anchor(b"space-1", merkle.empty_root(), epoch_round=(2).to_bytes(8, "big"))
...
def test_global_anchor_binds_the_epoch_round():
...
def test_verify_chain_rejects_rewound_rounds_even_when_hashes_are_consistent():
    ...
    g._entries.append(GlobalReceipt(index=1, owner_id=owner, anchored_root=root,
                                    epoch_round=lo, entry_hash=eh, prev_hash=e0.entry_hash))
...
def test_accountable_commits_and_is_provable_later():
    ...
    root = led.root
    g.anchor(led.owner_id, root, epoch_round=(7).to_bytes(8, "big"))
