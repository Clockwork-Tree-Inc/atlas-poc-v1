...
def test_timestamp_anchored():
    ...
    w.capture(b"c1", rb)
    chunks = [o for k, o in events if k == "chunk"]
    assert chunks[0].beacon_epoch_round == ra.epoch_round()
    assert chunks[1].beacon_epoch_round == rb.epoch_round()
...
def test_tamper_evident_altered_chunk_detected():
    ...
    bad = chunks[1]
    tampered = ForensicChunk(seq=bad.seq, cause=bad.cause, beacon_epoch_round=bad.beacon_epoch_round,
