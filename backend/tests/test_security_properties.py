...
def _epoch(A, B, *, lk, epoch_key, epoch_round, beacon_label):
    from atlas.session import establish_hybrid_tunnel
    A.advance_epoch_present(lk=lk, epoch_key=epoch_key, epoch_round=epoch_round)
    B.advance_epoch_present(lk=lk, epoch_key=epoch_key, epoch_round=epoch_round)
...
def test_forward_secrecy_later_key_cannot_read_earlier_epoch_ciphertext():
    ...
    t_e = _epoch(A, B, lk=b"L1" * 16, epoch_key=re.randomness, epoch_round=re.epoch_round(),
                 beacon_label=b"comp|" + re.epoch_round())
    msg_e = seal(b"epoch-e secret", mode=SendMode.NORMAL, key=t_e)
    # epoch e+1: beacon advances, tunnel re-keys
    re1 = beacon.round_at(5.0)
    t_e1 = _epoch(A, B, lk=b"L2" * 16, epoch_key=re1.randomness, epoch_round=re1.epoch_round(),
                  beacon_label=b"comp|" + re1.epoch_round())
    assert t_e1 != t_e
...
def test_replayed_recognition_is_rejected_after_beacon_advance():
    ...
    beacon = LocalBeacon(period_s=3.0)
    re = beacon.round_at(1.0)
    captured = _epoch(A, B, lk=b"L1" * 16, epoch_key=re.randomness, epoch_round=re.epoch_round(),
                      beacon_label=b"comp|" + re.epoch_round())
    # beacon advances; new epoch traffic is sealed under the new tunnel key
    re1 = beacon.round_at(5.0)
    fresh = _epoch(A, B, lk=b"L2" * 16, epoch_key=re1.randomness, epoch_round=re1.epoch_round(),
                   beacon_label=b"comp|" + re1.epoch_round())
...
def test_recognition_is_constant_within_epoch_but_changes_with_beacon():
    ...
    inp = dict(lk=b"L" * 32, epoch_key=b"E" * 32, epoch_round=b"\x00" * 8)
...
def test_off_device_rooting_session_key_requires_beacon_and_lk():
    ...
    common = dict(pole_value=b"Q" * 32, prev_key=b"\x00" * 32,
                  context_separator=CONTEXT_TUNNEL, epoch_round=b"\x00" * 8)
...
def test_containment_session_inert_after_liveness_break():
    ...
    A, _ = _pair()
    A.advance_epoch_present(lk=b"L" * 32, epoch_key=b"E" * 32, epoch_round=b"\x00" * 8)
    ...
    pole = g.state(sensor_digest=b"d", epoch_round=b"\x00" * 8)
...
def test_session_key_roles_do_not_share_one_secret():
    ...
    pole = PoLEState(p_live=1.0, state_digest=b"d" * 32, epoch_round=b"\x00" * 8, operate=True)
    A.continuity_tick(pole, epoch_round=b"\x00" * 8, beacon=b"bcn")
