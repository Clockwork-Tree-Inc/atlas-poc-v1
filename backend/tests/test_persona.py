...
def test_two_personas_are_mutually_unlinkable():
    tree = _tree()
    real = open_persona(tree, "alice", PW, tier=PseudonymTier.PUBLIC)
...
def test_same_username_different_tier_is_a_different_persona():
    tree = _tree()
    pub = open_persona(tree, "alice", PW, tier=PseudonymTier.PUBLIC)
    anon = open_persona(tree, "alice", PW, tier=PseudonymTier.ANONYMOUS)
...
def test_feature_slices_do_not_cross_link_between_personas():
    tree = _tree()
    a = open_persona(tree, "alice", PW, tier=PseudonymTier.PUBLIC)
...
def test_persona_selector_salt_is_per_user():
    """3d fix: the scrypt stretch must depend on the username (per-user salt), else ONE
    precomputed table cracks every account. Recompute the OLD salt-only construction and assert
    the shipped selector no longer matches it -> the username now enters scrypt."""
    import hashlib
    from atlas.crypto.primitives import H
    from atlas.persona.persona import (
        persona_selector, _normalize_username, PERSONA_SELECTOR_SALT,
        _SCRYPT_N, _SCRYPT_R, _SCRYPT_P, _SCRYPT_MAXMEM)
    u, pw = "alice", "correct horse battery"
    name = _normalize_username(u)
    old_stretched = hashlib.scrypt(pw.encode("utf-8"), salt=PERSONA_SELECTOR_SALT,
                                   n=_SCRYPT_N, r=_SCRYPT_R, p=_SCRYPT_P, dklen=32,
                                   maxmem=_SCRYPT_MAXMEM)
    old_selector = H(b"atlas/persona-selector", name.encode("utf-8"), old_stretched)
    assert persona_selector(u, pw) != old_selector          # username now folded into scrypt
    assert persona_selector(u, pw) == persona_selector(u, pw)  # still deterministic
