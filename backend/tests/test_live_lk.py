...
def test_both_devices_derive_the_same_lk_regardless_of_order():
    ...
    epoch = b"\x00" * 8
    lk_from_a_view = co_derive_lk([a, b], epoch_round=epoch)   # A holds [a, b]
    lk_from_b_view = co_derive_lk([b, a], epoch_round=epoch)   # B holds [b, a]
...
def test_neither_contribution_equals_or_reveals_the_lk():
    a, b = device_contribution(), device_contribution()
    lk = co_derive_lk([a, b], epoch_round=b"\x00" * 8)
    ...
    assert co_derive_lk([a2, b], epoch_round=b"\x00" * 8) != lk


def test_lk_is_epoch_bound():
    a, b = device_contribution(), device_contribution()
    assert co_derive_lk([a, b], epoch_round=b"\x00" * 8) != co_derive_lk([a, b], epoch_round=b"\x01" * 8)


def test_single_contribution_is_refused():
    with pytest.raises(ValueError):
        co_derive_lk([device_contribution()], epoch_round=b"\x00" * 8)
