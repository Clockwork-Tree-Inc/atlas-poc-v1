...
from atlas.crypto.sign import generate_sig_keypair, keypair_from_seed, sign
from atlas.keys.identity import (
    build_identity_tree,
    reconstruct_user_half,
    split_user_half_for_recovery,
    verify_one_to_one,
    attest_biometric_match,
)
...
def test_full_engine_n_phone_backend_live_lk(n):
    ...
    epoch_round, epoch_key, beacon_t = rnd.epoch_round(), rnd.randomness, rnd.randomness
    ...
    live_lk = co_derive_lk(contribs, epoch_round=epoch_round)
    for p in people:                                   # each computes it from its own view
        assert co_derive_lk([p.contribution] + [q.contribution for q in people if q is not p],
                            epoch_round=epoch_round) == live_lk
    ...
    for p in people:
        ...
        matcher = generate_sig_keypair()
        att = attest_biometric_match(matcher, asserted_handle=child.handle, challenge=chal)
        ok = verify_one_to_one(asserted_handle=child.handle, revealed_public=child.public,
                               challenge=chal, signature=sig, matcher_public=matcher.public,
                               match_attestation=att)
        ...
        tampered = verify_one_to_one(asserted_handle=child.handle, revealed_public=child.public,
                                     ...
                                     matcher_public=matcher.public, match_attestation=att)
        assert not tampered.signature_valid

        # session: advance AGAINST the live co-derived LK (presence-gated unwrap chain)
        p.session = p.device.advance_epoch_present(lk=live_lk, epoch_key=epoch_key, epoch_round=epoch_round)
    ...
    node.register_witness_public(epoch_round=epoch_round,
                                 pub=keypair_from_seed(_witness_seed(live_lk, epoch_round)).public)
    for signer in people:
        ...
        for verifier in people:
            ...
            reg = PublicWitnessRegistry(); reg.register_public(vb.epoch_round, node.witness_public(vb.epoch_round))
    ...
    for i, snd in enumerate(people):
        for j, rcv in enumerate(people):
            ...
            s_seed = seed_chain(channel_key=pair_key_snd, lk=live_lk, epoch_round=epoch_round, direction=direction)
            r_seed = seed_chain(channel_key=pair_key_rcv, lk=live_lk, epoch_round=epoch_round, direction=direction)
            snd_chain, rcv_chain = FSChain(s_seed, epoch_round=epoch_round), FSChain(r_seed, epoch_round=epoch_round)
