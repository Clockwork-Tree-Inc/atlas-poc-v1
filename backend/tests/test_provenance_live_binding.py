...
def _capture(*, lk, session_key, registry, tree=None, content=b"frame", rnd=None, ledger=None,
    ...
    if publish:
        registry.publish(lk, rnd.epoch_round())          # server publishes the epoch witness pub
    bundle = sign_capture(
...
def test_backdated_attribution_rejected():
    ...
    registry.publish(lk_b, rnd_b.epoch_round())                    # both epochs published
    ...
    bundle.epoch_round = rnd_b.epoch_round()
...
def test_recipient_can_verify_without_lk():
    ...
    pub = registry.witness_pub(rnd.epoch_round())
