...
from atlas.keys.identity import attest_biometric_match, build_identity_tree, verify_one_to_one
...
def _sk(prev=b"\x00" * 32):
    return derive_session_key_decoupled(
        ...
        prev_key=prev, context_separator=b"tunnel", epoch_round=b"\x00" * 8,
    )
...
def test_ratchet_one_way_forward_secrecy():
    k0 = os.urandom(32)
    k1 = ratchet(k0, entropy_t=b"e1", beacon_t=b"b1", epoch_round=b"\x00" * 8)
    k2 = ratchet(k1, entropy_t=b"e2", beacon_t=b"b2", epoch_round=b"\x00" * 8)
    assert k0 != k1 != k2
    # deterministic given the same inputs
    assert k1 == ratchet(k0, entropy_t=b"e1", beacon_t=b"b1", epoch_round=b"\x00" * 8)
...
def test_pseudonym_tiers_are_user_selected_and_unlinkable():
    ...
    assert build_identity_tree(seed).pseudonym("shop", PseudonymTier.PUBLIC).handle == pub.handle
...
def test_one_to_one_verification():
    ...
    # the trusted matcher (SE / recovery attestor) signs the challenge-bound match verdict
    ...
    ok = verify_one_to_one(
        ...
        challenge=chal, signature=sig, matcher_public=matcher.public, match_attestation=att,
    )
    assert ok.ok
    ...
    # no matcher attestation => biometric not proven => rejected (can't fake it with a bool)
    nobio = verify_one_to_one(
        asserted_handle=child.handle, revealed_public=child.public,
        challenge=chal, signature=sig,
    )
    assert not nobio.ok
...
def _enrol():
    ...
    enr = R.enrol_recovery(tree, seed, bio, device=device, passcode="hunter2")
...
def test_device_present_card_path_uses_enclave():
    ...
    assert rec.root_handle == tree.root_handle   # recovery rebuilt the same durable identity
