...
def _live_pole(beacon_round):
    ...
    return g.state(sensor_digest=b"sensor", epoch_round=beacon_round.epoch_round())
...
def _sign(registry, **kw):
    ...
    """
    registry.publish(kw["lk"], kw["beacon_round"].epoch_round())
...
def test_capstone_sign_and_verify_roundtrip():
    ...
    assert bundle.epoch_round == rnd.epoch_round()
...
def test_signing_requires_verified_live_author():
    ...
    for _, (psl, psnl) in spoof_stream(40):
        g.update(p_s_given_live=psl, p_s_given_not_live=psnl)
    spoof_pole = g.state(sensor_digest=b"d", epoch_round=rnd.epoch_round())
