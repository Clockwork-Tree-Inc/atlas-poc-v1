...
def test_reroot_forward_heals_and_keeps_durable_tsk():
    ...
    new_tree = reroot_system_id(tree, tsk_seed=seed, user_authorized=True)
    ...
    assert new_tree.system_id_handle() != new_tree.root_handle
...
def test_holder_authority_no_operator_reroot():
    ...
    with pytest.raises(OperatorForbidden):
        reroot_system_id(tree, tsk_seed=os.urandom(32), user_authorized=False)
