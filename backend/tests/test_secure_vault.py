...
def _live_pole(epoch=b"\x00" * 8):
    ...
    return g.state(sensor_digest=b"s", epoch_round=epoch)


def _dead_pole(epoch=b"\x00" * 8):
    return PoLEState(p_live=0.0, state_digest=b"d", epoch_round=epoch, operate=False)
...
def test_put_get_roundtrip_under_presence():
    ...
    v.put("note", b"my seed phrase", live_biometric=BIO, pole=_live_pole(b.epoch_round()), beacon_round=b)
    assert v.get("note", live_biometric=BIO, pole=_live_pole(b.epoch_round())) == b"my seed phrase"
...
def test_wrong_biometric_fails_closed():
    v, _ = _vault()
    b = LocalBeacon().round_at(1.0)
    v.put("x", b"data", live_biometric=BIO, pole=_live_pole(b.epoch_round()), beacon_round=b)
    with pytest.raises(NotPresent):
        v.get("x", live_biometric=b"\x00" * 64, pole=_live_pole(b.epoch_round()))   # not the enrolled finger


def test_at_rest_is_unreadable_brick():
    v, _ = _vault()
    b = LocalBeacon().round_at(1.0)
    v.put("x", b"PLAINTEXT-SECRET", live_biometric=BIO, pole=_live_pole(b.epoch_round()), beacon_round=b)
...
def test_provenance_stamp_binds_author_and_detects_tampering():
    v, tree = _vault()
    b = LocalBeacon().round_at(1.0)
    v.put("doc", b"authored content", live_biometric=BIO, pole=_live_pole(b.epoch_round()), beacon_round=b)
    item = v._store["doc"]
    ...
    with pytest.raises(Exception):
        v.get("doc", live_biometric=BIO, pole=_live_pole(b.epoch_round()))
...
def test_phone_only_refuses_export():
    ...
    with pytest.raises(BackupNotEnabled):
        v.export_backup(kem.generate_keypair().public, live_biometric=BIO, pole=_live_pole(b.epoch_round()))


def test_noncustodial_backup_host_blind_recovery_restores():
    v, _ = _vault(BackupChoice.NONCUSTODIAL)
    b = LocalBeacon().round_at(1.0)
    v.put("k", b"backed-up-secret", live_biometric=BIO, pole=_live_pole(b.epoch_round()), beacon_round=b)
    recovery = kem.generate_keypair()
    blob = v.export_backup(recovery.public, live_biometric=BIO, pole=_live_pole(b.epoch_round()))
