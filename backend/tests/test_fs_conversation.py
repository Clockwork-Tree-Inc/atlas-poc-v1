...
def _seed(direction):
    return seed_chain(channel_key=b"K" * 32, lk=b"L" * 32, epoch_round=b"\x00" * 8, direction=direction)


def test_roundtrip_both_directions_in_lockstep():
    epoch, beacon = b"\x00" * 8, b"beacon-1"
    a_send = FSChain(_seed(b"A->B"), epoch_round=epoch)
    b_recv = FSChain(_seed(b"A->B"), epoch_round=epoch)     # B's copy of the A->B chain
...
def test_forward_secrecy_leaked_later_key_cannot_read_earlier():
    ...
    chain = derive_chain(seed, count=8, beacon_t=beacon, epoch_round=epoch)
    ...
    leaked_ck = chain[5][1]
    forward = derive_chain(leaked_ck, count=3, beacon_t=beacon, epoch_round=epoch)
