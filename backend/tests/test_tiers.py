...
# --- user's-choice recovery policy (require the SE card for guardian recovery) ---

def test_default_policy_matches_design_of_record():
    # default (no card required): a guardian quorum + name/password reaches social with no card.
    f = AvailableFactors(guardian_quorum=True, name_password=True, recovery_card=False)
    assert RecoveryTier.SOCIAL in reachable_tiers(f)                    # DEFAULT_POLICY
    assert RecoveryTier.SOCIAL in reachable_tiers(f, RecoveryPolicy())  # explicit default


def test_policy_card_required_blocks_social_without_card():
    # the user opts to require the SE recovery card for guardian recovery. A guardian quorum +
    # name/password but NO card cannot reach social — it drops to the in-person floor.
    policy = RecoveryPolicy(require_card_for_social=True)
    f = AvailableFactors(guardian_quorum=True, name_password=True, recovery_person=True,
                         recovery_card=False)
    assert RecoveryTier.SOCIAL not in reachable_tiers(f, policy)
    assert select_tier(f, policy) is RecoveryTier.PHYSICAL_SELF


def test_policy_card_required_social_reachable_with_card():
    policy = RecoveryPolicy(require_card_for_social=True)
    f = AvailableFactors(guardian_quorum=True, name_password=True, recovery_card=True)
    assert RecoveryTier.SOCIAL in reachable_tiers(f, policy)
    assert select_tier(f, policy) is RecoveryTier.SOCIAL


def test_policy_never_affects_the_floor():
    # a card is never required to be yourself in person — the floor holds under any policy.
    policy = RecoveryPolicy(require_card_for_social=True)
    f = AvailableFactors(name_password=True, recovery_person=True, recovery_card=False)
    assert select_tier(f, policy) is RecoveryTier.PHYSICAL_SELF
    assert never_locked_out(f) is True
