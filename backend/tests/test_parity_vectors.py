...
def test_ratchet_vectors():
    for v in _load()["ratchet"]:
        out = ratchet(b(v["prev"]), entropy_t=b(v["entropy"]), beacon_t=b(v["beacon"]), epoch_round=b(v["epoch_round"]))
        assert out == b(v["output"])
...
def test_session_key_vectors():
    for v in _load()["session_key_decoupled"]:
        sk = derive_session_key_decoupled(
            ...
            prev_key=b(v["prev_key"]), context_separator=b(v["context_separator"]), epoch_round=b(v["epoch_round"]))
...
def test_presence_unwrap_chain_vectors():
    for v in _load()["presence_unwrap_chain"]:
        ...
        assert unwrap_epoch_key(b(v["wrapped_epoch_key"]),
                                presence_secret=b(v["enrollment_secret"]), epoch_round=eid) == b(v["epoch_key"])
        assert unlock_lk(b(v["wrapped_lk"]), epoch_key=b(v["epoch_key"]), epoch_round=eid) == b(v["lk"])
...
def test_live_provenance_binding_vectors():
    for v in _load()["live_provenance_binding"]:
        assert _witness_seed(b(v["lk"]), b(v["epoch_round"])) == b(v["witness_seed"])
        sc = _session_commit(b(v["session_key"]), b(v["content_hash"]))
        assert sc == b(v["session_commit"])
        core = _attribution_core(b(v["content_hash"]), b(v["epoch_round"]), b(v["authorship_handle"]), sc)
...
def test_forensic_event_chain_vectors():
    ...
    for v in _load()["forensic_event_chain"]:
        ev = ForensicEvent(
            seq=v["seq"], prev_hash=b(v["prev_hash"]), epoch_round=b(v["epoch_round"]),
