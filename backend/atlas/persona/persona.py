...
def persona_selector(username: str, password: str, *,
    ...
    # PER-USER SALT: fold the normalised username INTO the scrypt salt so each account's stretch
    # is independent. With only the deployment-wide `salt`, an attacker who steals the store
    # computes the scrypt table ONCE and tests it against every account — cracking 10k users costs
    # about what cracking one costs, which is exactly what a per-user salt exists to prevent.
    stretched = hashlib.scrypt(password.encode("utf-8"), salt=salt + name.encode("utf-8"),
