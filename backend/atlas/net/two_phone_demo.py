...
from ..beacon import ArrivalTiming, EpochBeacon, LocalBeacon
...
from .node_server import mailbox_for
...
def _live_pole(rnd):
    ...
    return g.state(sensor_digest=b"s", epoch_round=rnd.epoch_round())
...
def run(node) -> List[str]:
    ...
    # Mailboxes are OPAQUE per-persona pseudonyms (messaging-feature handles), NOT
    # human-chosen names: two personas of one human get unlinkable mailboxes.
    mbox_a = mailbox_for(build_identity_tree(os.urandom(32)).profile("phone-a"))
    mbox_b = mailbox_for(build_identity_tree(os.urandom(32)).profile("phone-b"))
    node.register(mailbox=mbox_a, kem_pub={"mlkemEK": _b64(a_kp.public.mlkem_ek),
                                            "x25519PK": _b64(a_kp.public.x25519_pk)})
    node.register(mailbox=mbox_b, kem_pub={"mlkemEK": _b64(b_kp.public.mlkem_ek),
                                           "x25519PK": _b64(b_kp.public.x25519_pk)})
    say("Phone A and Phone B registered OPAQUE per-persona mailboxes with the node.")

    # -- A establishes the A-B key (node cannot derive it) --
    enc = kem.encapsulate(b_kp.public)          # A -> B
    ab_key = enc.shared
    node.relay_send(frm=mbox_a, to=mbox_b,
    ...
    node.relay_send(frm=mbox_a, to=mbox_b, blob_b64=_b64(seal(secret, mode=SendMode.NORMAL, key=ab_key).ciphertext))
    ...
    # The epoch (the epoch key) comes from the aggregator's QRNG EpochBeacon; the external-drand
    # stand-in (LocalBeacon) is bound in only as the bootstrap / defence-in-depth anchor.
    _boot = LocalBeacon().round_at(1.0)
    rnd = EpochBeacon().fire(ArrivalTiming([0.0, 0.03, 0.07]), anchor=_boot.epoch_round())
    ...
    a_witness_pub = keypair_from_seed(_witness_seed(lk, rnd.epoch_round())).public
    node.register_witness_public(epoch_round=rnd.epoch_round(), pub=a_witness_pub)
    ...
    node.relay_send(frm=mbox_a, to=mbox_b, blob_b64=_b64(seal(payload, mode=SendMode.NORMAL, key=ab_key).ciphertext))
    ...
    fetched = node.relay_fetch(mailbox=mbox_b)["messages"]
    got_msg = None
    verdict_line = None
    for env in fetched:
        ...
        if opened.startswith(b"{"):                  # the JSON photo payload
            ...
            wp = node.witness_public(b_bundle.epoch_round)   # public half from the anchor
            if wp is not None:
                b_reg.register_public(b_bundle.epoch_round, wp)
