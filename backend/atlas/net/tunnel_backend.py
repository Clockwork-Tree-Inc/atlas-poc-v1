...
def serve(host: str = "0.0.0.0", port: int = 8787) -> None:  # nosec B104 - intentional LAN bind: LAN phones reach the tunnel backend; override with --host
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):  # noqa: N802
            try:
                ...
                return
            code, obj = backend.dispatch("POST", self.path, body)
            self._send(code, obj)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--host", default="0.0.0.0")  # nosec B104 - LAN bind so phones can reach the backend; pass --host 127.0.0.1 to restrict
