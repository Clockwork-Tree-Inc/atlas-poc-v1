...
def _attest_msg(commitment: int, context: bytes) -> bytes:
    """What an attester (the ring / Secure Enclave that MEASURED the score) signs — binding the
    commitment to the score to this context, so the committed `w` can't be prover-invented."""
    return H(b"atlas/liveness/attest", _i2osp(commitment), context)
...
class LivenessProof:
    ...
    attestation: bytes = b""          # optional attester signature over the commitment (see _attest_msg)


def prove_liveness(score: int, threshold: int, *, bits: int = 32,
                   context: bytes = b"", attester: Optional[HybridSigKeypair] = None) -> LivenessProof:
    ...
    attestation = sign(attester, _attest_msg(commitment, context)) if attester is not None else b""
    return LivenessProof(threshold=threshold, bits=bits, context=context,
                         commitment=commitment, bit_commitments=bit_commitments,
                         bit_proofs=bit_proofs, attestation=attestation)


def verify_liveness(pf: LivenessProof, *, required_threshold: int, expected_context: bytes = b"",
                    attester_public: Optional[HybridSigPublic] = None) -> bool:
    """Verify a liveness proof at a VERIFIER-CHOSEN bar. Returns True iff: the proof's threshold is
    at least the verifier's `required_threshold` (so the prover cannot pick its own low/zero bar);
    the proof is bound to `expected_context` (bind a fresh epoch+nonce here so a proof cannot be
    replayed across sessions/verifiers); every bit is proven ∈ {0,1}; and the committed value
    reconstructs the claimed commitment — i.e. the hidden score ≥ the required threshold.

    NOTE (tracked follow-on): the committed `score` is still prover-supplied — binding it to an
    ATTESTED measurement (a ring/SE signature over Ped(score,·) / the PoLE_state) closes the last
    gap, and needs the attestation seam wired in.
    """
    if pf.threshold < required_threshold:
        return False                       # verifier's floor — NOT the prover's chosen threshold
    if pf.context != expected_context:
        return False                       # freshness / anti-replay: bind a fresh epoch + nonce
    if attester_public is not None:
        # require the committed score to be ATTESTED (ring/SE-signed) — not prover-invented
        if not pf.attestation or not verify(attester_public, _attest_msg(pf.commitment, pf.context), pf.attestation):
            return False
