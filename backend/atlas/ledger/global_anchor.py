...
class GlobalReceipt:
    ...
    epoch_round: bytes        # decentralized timekeeper binding
    entry_hash: bytes
    prev_hash: bytes
...
class GlobalAnchorLog:
    ...
    def anchor(self, owner_id: bytes, root: bytes, epoch_round: bytes) -> GlobalReceipt:
        # drand rounds only move forward — reject a backdated (non-monotonic) round so timestamps
        # cannot be rewound.
        r = int.from_bytes(epoch_round, "big")
        if self._entries and r < self._last_round:
            raise ValueError("epoch_round must be non-decreasing (no backdating)")
        prev = self.head
        idx = len(self._entries)
        # length-prefix EVERY variable-length field (owner_id, root, epoch_round) so no byte can
        ...
        entry_hash = H(_GLOBAL, prev, _lp(owner_id), _lp(root), _lp(epoch_round),
                       idx.to_bytes(8, "big"))
        receipt = GlobalReceipt(index=idx, owner_id=owner_id, anchored_root=root,
                                epoch_round=epoch_round, entry_hash=entry_hash, prev_hash=prev)
    ...
    def verify_chain(self) -> bool:
        ...
        for i, e in enumerate(self._entries):
            expect = H(_GLOBAL, prev, _lp(e.owner_id), _lp(e.anchored_root), _lp(e.epoch_round),
                       i.to_bytes(8, "big"))
            if e.entry_hash != expect or e.prev_hash != prev or e.index != i:
                return False
            r = int.from_bytes(e.epoch_round, "big")
            if r < last_round:
