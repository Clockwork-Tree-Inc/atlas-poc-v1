...
def cast_vote_proof(voter: HybridSigKeypair, proof, *, up: bool, epoch: int, nonce: bytes) -> Vote:
    """SYBIL-RESISTANT vote — one per HUMAN per target (NOT one per persona), enforced in ZK.

    The voter PRESENTS a `PersonTagProof` whose scope is the target being voted on. The proof is
    verified (a zero-knowledge binding to a credentialed root + freshness — the root is NEVER seen),
    and the vote is DEDUPED on the verified person-tag `N`, while still SIGNED by the persona's vote
    key (so the vote stays attributable/blockable). So one human casts one vote per target no matter
    how many personas they hold, anonymously, with `N` unlinkable across targets. Raises on an
    invalid or stale proof. Use for anything COUNTED (votes/reviews/polls/UBI)."""
    from ..zk.person_tag import verify_person_tag
    if not verify_person_tag(proof, expected_epoch=epoch, expected_nonce=nonce):
        raise ValueError("invalid or stale person-tag proof")
    nul = proof.N.to_bytes((proof.N.bit_length() + 7) // 8 or 1, "big")   # the person-tag N as the dedupe key
    return cast_vote(voter, target=proof.scope, value=LIKE if up else DISLIKE, nullifier=nul, epoch=epoch)
