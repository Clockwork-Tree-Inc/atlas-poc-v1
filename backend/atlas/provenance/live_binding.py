...
def _witness_seed(lk: bytes, epoch_round: bytes) -> bytes:
    # The witness signing key is derived from the CURRENT LK (secret) + epoch.
    return H(b"atlas/lk-witness", lk, epoch_round)
...
def _attribution_core(content_hash: bytes, epoch_round: bytes, authorship_handle: bytes,
                      session_commit: bytes) -> bytes:
    return H(b"atlas/prov/attribution-core", content_hash, epoch_round,
...
class PublicWitnessRegistry:
    ...
    def publish(self, lk: bytes, epoch_round: bytes) -> HybridSigPublic:
        """Server side: derive the epoch witness keypair from the LK and publish
        its PUBLIC half (the LK itself never leaves the HSM)."""
        kp = keypair_from_seed(_witness_seed(lk, epoch_round))
        if epoch_round not in self._pub:
            self._order.append(epoch_round)
            self._head = H(b"atlas/witness-chain", self._head, epoch_round, kp.public.encode())
        self._pub[epoch_round] = kp.public
        return kp.public

    def register_public(self, epoch_round: bytes, pub: HybridSigPublic) -> None:
        ...
        if epoch_round not in self._pub:
            self._order.append(epoch_round)
            self._head = H(b"atlas/witness-chain", self._head, epoch_round, pub.encode())
        self._pub[epoch_round] = pub

    def witness_pub(self, epoch_round: bytes) -> Optional[HybridSigPublic]:
        return self._pub.get(epoch_round)

    def position(self, epoch_round: bytes) -> Optional[int]:
        return self._order.index(epoch_round) if epoch_round in self._order else None


def bind_live_provenance(*, lk: bytes, session_key: bytes, content_hash: bytes,
                         epoch_round: bytes, authorship_handle: bytes) -> LiveProvenanceBinding:
    ...
    core = _attribution_core(content_hash, epoch_round, authorship_handle, session_commit)
...
def verify_live_provenance(binding: Optional[LiveProvenanceBinding], *, content_hash: bytes,
                           epoch_round: bytes, authorship_handle: bytes,
    ...
    if binding is None:
        return False
    pub = registry.witness_pub(epoch_round)
    if pub is None:
        return False
    core = _attribution_core(content_hash, epoch_round, authorship_handle, binding.session_commit)
