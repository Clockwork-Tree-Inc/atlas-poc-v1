...
def _capture_binding(label: bytes, authorship_handle: bytes, content_hash: bytes,
                     epoch_round: bytes) -> bytes:
    return H(label, authorship_handle, content_hash, epoch_round)
...
def sign_capture(
    *,
    ...
    epoch_round = beacon_round.epoch_round()
    ...
    live_challenge = _capture_binding(_LIVENESS_BIND, authorship.handle, content_hash, epoch_round)
    ...
    if verification_credential is not None:
        vnonce = _capture_binding(_INHERITED_BIND, authorship.handle, content_hash, epoch_round)
    ...
    live_binding = bind_live_provenance(
        ...
        epoch_round=epoch_round, authorship_handle=authorship.handle)
...
def verify_provenance(
    ...
    *,
    ...
    expected_live_challenge = _capture_binding(
        _LIVENESS_BIND, bundle.authorship_handle, bundle.content_hash, bundle.epoch_round)
    liveness_ok = (
        bundle.liveness.verify()
        and bundle.liveness.operate
        and bundle.liveness.epoch_round == bundle.epoch_round
    ...
    if required_level >= AssuranceLevel.L1:
        expected_vnonce = _capture_binding(
            _INHERITED_BIND, bundle.authorship_handle, bundle.content_hash, bundle.epoch_round)
    ...
    live_provenance_ok = verify_live_provenance(
        bundle.live_binding, content_hash=bundle.content_hash, epoch_round=bundle.epoch_round,
