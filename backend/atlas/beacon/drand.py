...
External drand is NOT the epoch key (that is the aggregator's QRNG `beacon.epoch`).
It is Atlas's BOOTSTRAP public timeline (before the epoch beacon has fired) and its
DEFENCE-IN-DEPTH ANCHOR (an independent, party-neutral, court-verifiable round bound
alongside the epoch). This client fetches rounds as-is from a League-of-Entropy relay
and exposes the same `Beacon` interface as `LocalBeacon`, so the rest of the protocol
is agnostic to which is wired in.
...
# Pinned TIME ROOT. genesis + period are SOURCE CONSTANTS, like the chain hash and group
# key — never taken from the relay. A relay that lied about genesis would shift every
# timestamp by a constant offset while every signature still verifies (authenticity is over
# the round INDEX, not the wall-clock time), yielding perfectly-verified provenance records
# uniformly off by that offset. Values are quicknet's, from the drand API.
QUICKNET_GENESIS_TIME = 1692803367.0
QUICKNET_PERIOD_S = 3.0
...
class DrandHTTPBeacon:
    ...
    def __init__(
        ...
        *,
        ...
        period_s: float = QUICKNET_PERIOD_S,
        genesis_time: float = QUICKNET_GENESIS_TIME,
    ...
    def info(self) -> dict:
        ...
        # genesis/period are PINNED source constants, NOT taken from the relay. If the relay
        # disagrees with the pinned time root, REJECT rather than adopt its values.
        r_period = float(info.get("period", self.period_s))
        r_genesis = float(info.get("genesis_time", self.genesis_time))
        if r_period != self.period_s or r_genesis != self.genesis_time:
            raise ValueError(
                "drand relay disagrees with the pinned time root "
                f"(period {r_period} vs {self.period_s}, genesis {r_genesis} vs {self.genesis_time}) — rejected")
        return info
    ...
    def _fetch(self, which: str) -> BeaconRound:
        ...
        # A relay must not walk around verification by OMITTING the signature: when
        # verification is on, a missing signature is a REJECTION, not a skip.
        if self._verify_bls and not rnd.signature:
            raise ValueError("drand response carried no signature — rejected (verification is mandatory)")
