...
The LK value is a CLEAN QRNG output (the fresh entropy core alone) — NEITHER the
inter-arrival timing digest NOR the external-drand anchor is mixed into the value
bytes. The timing only sets WHEN the QRNG next fires (`next_sampling_offset_s`).
Forward secrecy comes from the fresh core per firing plus the ratchet chain, not
from committing timing into the value. (`timing_commitment` is retained on the
draw for scheduling/audit, not as key material.)

External drand is NOT a value input: an `anchor` (an external-drand round) may be
recorded ALONGSIDE a draw for bootstrap / defence-in-depth, but it never enters
the clean-QRNG value. The aggregator's public epoch beacon (`beacon.epoch`) is
built ON TOP of these draws — it is the epoch key, not drand.
...
class TimedDraw:
    ...
    anchor: bytes            # OPTIONAL external-drand round, recorded for bootstrap /
                             # defence-in-depth ONLY — never folded into `randomness`
    randomness: bytes        # the entropy each device folds into its session key (CLEAN QRNG)
...
class ServerQRNG:
    ...
    def fire(self, arrival: ArrivalTiming, anchor: bytes = b"",
        ...
        `anchor` is an OPTIONAL external-drand round recorded on the draw for
        bootstrap / defence-in-depth. Like the timing, it NEVER enters the value —
        the epoch's public beacon (`beacon.epoch`) binds the anchor into its
        signature instead, so the value stays clean QRNG.

        `entropy_core` is normally drawn fresh from the CSPRNG. It is injectable
        ONLY so a test can hold the core constant and prove that neither the
        inter-arrival timing NOR the anchor changes the value (they time/annotate
        the firing, not the bytes). Production never passes it.
        """
        self._fire_count += 1
        ...
        randomness = sha256(b"atlas/qrng/value", core)
        ...
        return TimedDraw(
            anchor=anchor,              # recorded for bootstrap / defence-in-depth, NOT in the value
