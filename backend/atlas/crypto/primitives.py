...
def random_bytes(n: int) -> bytes:
    """The clean-value source. HONEST NAMING: this is the OS **CSPRNG** (getrandom/urandom),
    seeded by the platform's hardware TRNG — strong and unpredictable, but **NOT quantum**.
    "QRNG" throughout the codebase names the clean-value *seam/role* (per the spec's
    QRNG-everywhere intent); a real quantum source (a per-node QRNG dongle or a QRNG service)
    drops in behind this same seam later. Until then: CSPRNG-over-hardware-TRNG (QRNG-ready)."""
    return os.urandom(n)
