...
from ..keys.identity import Child, Profile, handle_of
...
def rp_authenticator(profile: Profile, relying_party: str) -> Child:
    """The PER-RELYING-PARTY authenticator key for a persona.

    Each relying party gets a DISTINCT keypair — and therefore a distinct handle AND a
    distinct public key — derived under the persona seed (`feature("rp:" + name)`). So
    two relying parties cannot correlate the same person by a shared authenticator key
    or handle (no cross-RP linkage), and two personas at the same RP are likewise
    unlinkable. This is the fix for reusing ONE global authorship key across every RP,
    where both the handle and the public key were identical everywhere = a cross-RP
    tracking vector. The key is deterministic per (persona, relying_party), so the RP
    still recognizes the same returning user."""
    return profile.feature("rp:" + relying_party)


def authenticate_to_rp(challenge: AuthChallenge, *, profile: Profile, pole: PoLEState,
                       yubikey: Optional[YubiKeyBio] = None,
                       fingerprint_matched: bool = False) -> VerifiedHumanAssertion:
    """Safe-by-construction authentication: derive the PER-RP authenticator for THIS
    challenge's relying party, so a caller cannot accidentally present one shared key
    across relying parties (which would make the person linkable across them). Register
    the matching `rp_authenticator(profile, rp_name)` handle/public with the RP."""
    authn = rp_authenticator(profile, challenge.relying_party)
    return authenticate(challenge, authorship=authn, pole=pole,
                        yubikey=yubikey, fingerprint_matched=fingerprint_matched)
