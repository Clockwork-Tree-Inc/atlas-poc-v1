...
# Minimum window for a trustworthy same-body decision. Below this, two DIFFERENT people can
# clear the floor by chance — measured on the MotionSense recordings (50 Hz): ~21% false-accept
# at 1 s (50 samples), 0% at 8 s (400). The limit is how long two gaits take to diverge, not the
# sample rate, so sampling faster cannot shorten it. Fail closed on short input.
SAME_BODY_MIN_SAMPLES = 400
...
def same_body(phone: Sequence[float], ring: Sequence[float], *,
    ...
    Fail-closed: a detached ring, an idle/flat ring, a different person, OR a window shorter
    than SAME_BODY_MIN_SAMPLES -> False."""
    if len(phone) < SAME_BODY_MIN_SAMPLES or len(ring) < SAME_BODY_MIN_SAMPLES:
        return False
