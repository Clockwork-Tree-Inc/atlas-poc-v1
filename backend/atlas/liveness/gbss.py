...
def pole_from_gbss(vectors: Sequence[EntropyVector], *, epoch_round: bytes,
                   captured_round: bytes | None = None, sensor_digest: bytes = b"gbss"):
    ...
    fail-closed. `captured_round` (security review #3) binds the batch to the CURRENT
    fresh beacon round: a replayed / pre-canned batch carries a stale round and
    fail-closes, so a recorded-varying feed can't read as live by entropy alone."""
