...
class LivenessAttestation:
    ...
    def _message(self) -> bytes:
        return self.message_for(self.epoch_round, self.pole_digest, self.operate, self.challenge)

    @staticmethod
    def message_for(epoch_round: bytes, pole_digest: bytes, operate: bool,
                    challenge: bytes = b"") -> bytes:
        # SECURITY: length-prefix every field (4-byte big-endian) so the signed
        # message is an INJECTIVE function of (epoch_round, pole_digest, operate,
        # challenge). A plain `|`-delimited concatenation is ambiguous — a 0x7c
        # byte inside epoch_round (raw beacon randomness) is an alternative split
        # point, letting one signature re-parse to a different epoch_round/challenge.
        # This mirrors hkdf_combine's length-prefixing discipline (primitives.py).
        flag = b"\x01" if operate else b"\x00"
        parts = [b"atlas/attest", epoch_round, pole_digest, flag, challenge]
...
class AttestationSubsystem:
    ...
    def attest(self, pole: PoLEState, *, challenge: bytes = b"") -> Optional[LivenessAttestation]:
        ...
        msg = LivenessAttestation.message_for(
            pole.epoch_round, pole.state_digest, pole.operate, challenge)
        sig = sign(self.enclave_key, msg)
        return LivenessAttestation(
            epoch_round=pole.epoch_round,
