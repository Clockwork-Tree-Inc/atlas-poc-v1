...
def seed_chain(*, channel_key: bytes, lk: bytes, epoch_round: bytes, direction: bytes) -> bytes:
    ...
    return hkdf_combine([channel_key, lk, epoch_round, direction], info=_CHAIN_INFO, length=32)


def _step(chain_key: bytes, *, beacon_t: bytes, epoch_round: bytes) -> Tuple[bytes, bytes]:
    ...
    mk = hkdf_combine([chain_key, b"mk", beacon_t, epoch_round], info=_CHAIN_INFO, length=32)
    ck = hkdf_combine([chain_key, b"ck", beacon_t, epoch_round], info=_CHAIN_INFO, length=32)
    return mk, ck
...
class FSChain:
    ...
    def __init__(self, seed: bytes, *, epoch_round: bytes):
        self._ck = seed
        self._epoch_round = epoch_round

    def seal(self, plaintext: bytes, *, beacon_t: bytes, aad: bytes = b"") -> bytes:
        mk, nxt = _step(self._ck, beacon_t=beacon_t, epoch_round=self._epoch_round)
...
def derive_chain(seed: bytes, *, count: int, beacon_t: bytes, epoch_round: bytes) -> List[Tuple[bytes, bytes]]:
    ...
    for _ in range(count):
        mk, nxt = _step(ck, beacon_t=beacon_t, epoch_round=epoch_round)
        out.append((mk, ck))
        ck = nxt
    return out
