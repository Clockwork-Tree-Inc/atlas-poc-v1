...
def _aad(direction: bytes, index: int, epoch_round: bytes) -> bytes:
    ...
    return H(b"atlas/conv/aad", direction, index.to_bytes(8, "big"), epoch_round)


def _sig_core(content: bytes, direction: bytes, index: int, epoch_round: bytes) -> bytes:
    ...
    return H(b"atlas/conv/sig-core", H(b"atlas/conv/content", content),
             direction, index.to_bytes(8, "big"), epoch_round)
...
class Envelope:
    ...
    def to_wire(self) -> bytes:
        return json.dumps({
            "i": self.index, "d": _b64(self.direction), "e": _b64(self.epoch_round),
    ...
    def from_wire(blob: bytes) -> "Envelope":
        o = json.loads(blob)
        return Envelope(index=o["i"], direction=_u(o["d"]), epoch_round=_u(o["e"]),
...
class _SendChain:
    ...
    def __init__(self, ck: bytes, *, epoch_round: bytes, beacon_t: bytes, index: int = 0):
        self._ck = ck
    ...
    def seal(self, plaintext: bytes, direction: bytes):
        mk, nxt = _step(self._ck, beacon_t=self._beacon_t, epoch_round=self.epoch_round)
        i = self._i
        self._ck = nxt                       # advance; old chain key discarded
        self._i += 1
        ct = aead_encrypt(mk, plaintext, aad=_aad(direction, i, self.epoch_round))
        return i, ct

    def snapshot(self) -> dict:
        return {"ck": _b64(self._ck), "epoch": _b64(self.epoch_round),
                "beacon": _b64(self._beacon_t), "i": self._i}

    @classmethod
    def restore(cls, s: dict) -> "_SendChain":
        return cls(_u(s["ck"]), epoch_round=_u(s["epoch"]), beacon_t=_u(s["beacon"]), index=s["i"])
...
class _RecvChain:
    ...
    def __init__(self, ck: bytes, *, epoch_round: bytes, beacon_t: bytes,
    ...
    def open(self, index: int, ciphertext: bytes, direction: bytes) -> bytes:
        if index < self._next:
            ...
            self._next += 1
        return aead_decrypt(mk, ciphertext, aad=_aad(direction, index, self.epoch_round))
    ...
    def restore(cls, s: dict) -> "_RecvChain":
        return cls(_u(s["ck"]), epoch_round=_u(s["epoch"]), beacon_t=_u(s["beacon"]),
...
class Conversation:
    ...
    def create(cls, *, mode: ConversationMode, my_direction: bytes, peer_direction: bytes,
               channel_key: bytes, lk: bytes, epoch_round: bytes, beacon_t: bytes,
               authorship: Optional[Child] = None,
               peer_public: Optional[HybridSigPublic] = None) -> "Conversation":
        s_seed = seed_chain(channel_key=channel_key, lk=lk, epoch_round=epoch_round, direction=my_direction)
        r_seed = seed_chain(channel_key=channel_key, lk=lk, epoch_round=epoch_round, direction=peer_direction)
        return cls(mode=mode, my_direction=my_direction, peer_direction=peer_direction,
                   send=_SendChain(s_seed, epoch_round=epoch_round, beacon_t=beacon_t),
                   recv=_RecvChain(r_seed, epoch_round=epoch_round, beacon_t=beacon_t),
    ...
    def send(self, plaintext: bytes) -> Envelope:
        ...
        if self._mode is ConversationMode.ACCOUNTABLE:
            core = _sig_core(plaintext, self._my_dir, index, self._send.epoch_round)
            sig = sign(self._authorship.keypair, core)
        return Envelope(index=index, direction=self._my_dir, epoch_round=self._send.epoch_round,
    ...
    def receive(self, env: Envelope) -> bytes:
        ...
        if self._mode is ConversationMode.ACCOUNTABLE:
            ...
            core = _sig_core(plaintext, env.direction, env.index, env.epoch_round)
