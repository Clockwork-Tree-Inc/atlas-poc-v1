...
class VaultStamp:
    ...
    def core(self) -> bytes:
        return H(b"atlas/vault-stamp", self.author_handle, self.content_hash, self.epoch_round)
...
class SecureVault:
    ...
    def put(self, name: str, data: bytes, *, live_biometric: bytes, pole: PoLEState,
        ...
        stamp_core = H(b"atlas/vault-stamp", self._author.handle, content_hash, beacon_round.epoch_round())
        stamp = VaultStamp(author_handle=self._author.handle, content_hash=content_hash,
                           epoch_round=beacon_round.epoch_round(), signature=sign(self._author.keypair, stamp_core))
