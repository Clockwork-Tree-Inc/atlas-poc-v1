...
def _pack(seq: int, prev_hash: bytes, epoch_round: bytes, decision: DecisionType,
          outcome: Outcome, risk: RiskLevel, context_handle: bytes,
          signal_digest: bytes) -> bytes:
    return b"".join([
        seq.to_bytes(8, "big"), _lp(prev_hash), _lp(epoch_round),
...
class ForensicEvent:
    ...
    def event_hash(self) -> bytes:
        return H(b"atlas/forensic/event",
                 _pack(self.seq, self.prev_hash, self.epoch_round, self.decision,
...
class ForensicLedger:
    ...
    def record(self, *, decision: DecisionType, outcome: Outcome, risk: RiskLevel,
               epoch_round: bytes, context_handle: bytes, signals: Signals) -> ForensicEvent:
        event = ForensicEvent(
            seq=len(self.entries), prev_hash=self._head, epoch_round=epoch_round,
            decision=decision, outcome=outcome, risk=risk,
            context_handle=context_handle, signal_digest=_signal_digest(signals))
        eh = event.event_hash()
        packed = _pack(event.seq, event.prev_hash, event.epoch_round, event.decision,
...
def decide_and_record(ledger: ForensicLedger, *, decision: DecisionType,
                      epoch_round: bytes, context_handle: bytes, signals: Signals,
    ...
    event = ledger.record(decision=decision, outcome=outcome, risk=risk,
                          epoch_round=epoch_round, context_handle=context_handle, signals=signals)
