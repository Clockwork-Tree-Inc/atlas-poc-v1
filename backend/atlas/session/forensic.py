...
class ForensicChunk:
    seq: int
    cause: str
    beacon_epoch_round: bytes       # timestamp anchor
    ...
    @staticmethod
    def compute_hash(seq: int, cause: str, epoch_round: bytes, randomness: bytes,
                     prev_hash: bytes, ciphertext: bytes) -> bytes:
        return H(b"atlas/forensic/link", seq.to_bytes(4, "big"), cause.encode(),
                 epoch_round, randomness, prev_hash, ciphertext)
...
class ForensicWindow:
    ...
    def capture(self, plaintext: bytes, beacon_round: BeaconRound) -> ForensicChunk:
        ...
        chunk_hash = ForensicChunk.compute_hash(self._seq, self._cause.value, epoch_round,
                                                rnd, self._head, ct)
        chunk = ForensicChunk(seq=self._seq, cause=self._cause.value, beacon_epoch_round=epoch_round,
...
def open_forensic_window(header: ForensicHeader, chunks: List[ForensicChunk],
    ...
    for i, c in enumerate(chunks, start=1):
        ...
        expect = ForensicChunk.compute_hash(c.seq, c.cause, c.beacon_epoch_round,
