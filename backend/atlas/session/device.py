...
class EpochInputs:
    ...
    epoch_round: bytes      # beacon round id
...
class Device:
    ...
    def wrap_epoch_key(self, epoch_key: bytes, epoch_round: bytes) -> bytes:
        ...
        return wrap_epoch_key(epoch_key, enrollment_secret=self._enrollment_secret, epoch_round=epoch_round)

    def advance_epoch(self, *, wrapped_epoch_key: bytes, wrapped_lk: bytes, epoch_round: bytes,
        ...
        try:
            epoch_key = unwrap_epoch_key(wrapped_epoch_key, presence_secret=secret, epoch_round=epoch_round)
        except Exception as exc:
            raise PresenceRequired("epoch-key unwrap failed (not the enrolled present device)") from exc
        try:
            lk = unlock_lk(wrapped_lk, epoch_key=epoch_key, epoch_round=epoch_round)
        except Exception as exc:
    ...
    def advance_epoch_present(self, lk: bytes, epoch_key: bytes, epoch_round: bytes,
        ...
        if pole is None:
            pole = PoLEState(p_live=1.0, state_digest=H(b"atlas/present", epoch_round),
                             epoch_round=epoch_round, operate=True)
        wrapped_lk = wrap_lk(lk, epoch_key=epoch_key, epoch_round=epoch_round)
        wrapped_epoch_key = self.wrap_epoch_key(epoch_key, epoch_round)
        return self.advance_epoch(
            wrapped_epoch_key=wrapped_epoch_key, wrapped_lk=wrapped_lk, epoch_round=epoch_round,
    ...
    def continuity_tick(self, pole: PoLEState, *, epoch_round: bytes, beacon: bytes,
        ...
        with self._continuity_key.borrow() as ck:
            advanced = ratchet(ck, entropy_t=entropy_t,
                               beacon_t=beacon, epoch_round=epoch_round)
    ...
    def message_ratchet_step(self, prev_msg_key: bytes, *, beacon_t: bytes,
                             epoch_round: bytes) -> tuple[bytes, bytes]:
        ...
        nxt = ratchet(prev_msg_key, entropy_t=entropy_t, beacon_t=beacon_t, epoch_round=epoch_round)
