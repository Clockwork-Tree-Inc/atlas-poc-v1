...
def _unwrap_key(enrollment_secret: bytes, epoch_round: bytes) -> bytes:
    return hkdf(ikm=enrollment_secret, info=b"atlas/epoch-unwrap|" + epoch_round, length=32)


def wrap_epoch_key(epoch_key: bytes, *, enrollment_secret: bytes, epoch_round: bytes) -> bytes:
    ...
    return aead_encrypt(_unwrap_key(enrollment_secret, epoch_round), epoch_key, aad=_UNWRAP_AAD)


def unwrap_epoch_key(wrapped: bytes, *, presence_secret: bytes, epoch_round: bytes) -> bytes:
    ...
    return aead_decrypt(_unwrap_key(presence_secret, epoch_round), wrapped, aad=_UNWRAP_AAD)
...
def _lk_key(epoch_key: bytes, epoch_round: bytes) -> bytes:
    return hkdf(ikm=epoch_key, info=b"atlas/lk-unlock|" + epoch_round, length=32)


def wrap_lk(lk: bytes, *, epoch_key: bytes, epoch_round: bytes) -> bytes:
    """Server side: wrap the private LK UNDER the (public) epoch key. Only a party
    holding the current epoch key can unlock the LK."""
    return aead_encrypt(_lk_key(epoch_key, epoch_round), lk, aad=_LK_AAD)


def unlock_lk(wrapped_lk: bytes, *, epoch_key: bytes, epoch_round: bytes) -> bytes:
    """Unlock the LK with the (unwrapped) epoch key. Raises if the epoch key is
    wrong — no epoch key -> no LK."""
    return aead_decrypt(_lk_key(epoch_key, epoch_round), wrapped_lk, aad=_LK_AAD)
