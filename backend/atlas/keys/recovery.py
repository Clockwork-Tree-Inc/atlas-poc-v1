...
def enrol_recovery(
    tree: IdentityTree, tsk_seed: bytes, biometric_template: bytes, *,
    device: SecureEnclave, passcode: str
) -> RecoveryEnrolment:
    """Split the TSK seed 2-of-3 and bind `share_bio` to the device Enclave
    (device-present). The seed is SUPPLIED here (held momentarily at genesis) — it
    is NOT read off the tree, which never retains it; post-genesis the seed lives
    only in these shares. Total-loss recovery uses the two PORTABLE shares (card +
    context) and needs no biometric release of `share_bio`."""
    share_card, share_bio, share_context = shamir.split(tsk_seed, n=3, k=2)
