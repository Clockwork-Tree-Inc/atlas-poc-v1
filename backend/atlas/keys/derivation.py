...
def derive_session_key_decoupled(
    *,
    ...
    return SessionKey(epoch_round=epoch_round, _key=bytearray(material))


def derive_session_key_coupled(
    *, tsk: bytes, dev_key: bytes, pole_state: bytes, beacon: bytes, epoch_round: bytes
) -> SessionKey:
...
def ratchet(prev_key: bytes, *, entropy_t: bytes, beacon_t: bytes, epoch_round: bytes) -> bytes:
    ...
    """
    return hkdf_combine(
        [prev_key, H(entropy_t), beacon_t, epoch_round], info=LABEL_RATCHET, length=32
    )
