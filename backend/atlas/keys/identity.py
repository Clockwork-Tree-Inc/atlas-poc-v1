...
class Profile:
    ...
    pseudonym ("nightowl") cannot be tied to the real you or to your other personas by
...
class IdentityTree:
    ...
    # SECURITY: the whole TSK seed is NEVER stored on the tree. It exists only
    # transiently at genesis (build_identity_tree derives from it, then discards);
    # post-genesis it lives ONLY in the 2-of-3 recovery shares. reroot/recovery are
    # HANDED the seed momentarily — they never read it off this object. (Previously a
    # plain `tsk_seed` field let anyone holding the tree recompute both halves and
    # bypass the HSM's non-exportability.)
    ...
    def profile(self, username: str, tier: PseudonymTier = PseudonymTier.ANONYMOUS) -> Profile:
        ...
        certify as the real, verified you; ANONYMOUS for a throwaway pseudonym ('nightowl')."""
        seed = hkdf(ikm=self._system_id_secret,
    ...
    def duress_persona(self, slot: int = 0) -> Profile:
        """A designated DECOY persona presented under duress (#40). Derived under a DISTINCT HKDF
        domain (`atlas/duress-profile/`) from real profiles, so it can NEVER collide with any real
        `profile(username)` regardless of the username chosen — yet it is a full, plausible
        compartment (its own vault / contacts / feed / keys), cryptographically UNLINKABLE to the
        blind System-ID and to every real persona. Fully cooperating under coercion reveals nothing
        about the root or the real personas (one-way HKDF). Deterministic per (user, slot) so the
        decoy is stable across sessions. Present ONLY this when unlocked via the duress code — never
        reassemble or surface the real personas in a duress session."""
        seed = hkdf(ikm=self._system_id_secret,
                    info=b"atlas/duress-profile/" + str(int(slot)).encode(), length=32)
        identity_seed = hkdf(ikm=seed, info=b"atlas/profile/identity", length=32)
        identity = Child(context=f"duress:{slot}", keypair=keypair_from_seed(identity_seed))
        return Profile(username=f"duress:{slot}", tier=PseudonymTier.ANONYMOUS, identity=identity, _seed=seed)
...
def verify_one_to_one(
    *,
    ...
    challenge: bytes,
    signature: bytes,
    matcher_public: Optional[HybridSigPublic] = None,
    match_attestation: bytes = b"",
) -> VerificationResult:
    ...
    3. The biometric match is a VERIFIABLE ATTESTATION, not a caller-supplied bool
       (security review #9). The trusted matcher — the on-device Secure Enclave, or an
       accountable live-recovery attestor at total loss — signs a challenge-bound match
       statement, and we verify that signature here. So a match cannot be asserted without
       the matcher's key signing THIS exact challenge; the trust rests structurally and
       verifiably on that matcher key, not on a forgeable flag. (Use `attest_biometric_match`
       to produce the attestation.)
    ...
    match_stmt = H(b"atlas/biometric-match-1to1", asserted_handle, challenge)
    biometric_matched = bool(
        matcher_public is not None and match_attestation
        and sig_verify(matcher_public, match_stmt, match_attestation)
    )
    return VerificationResult(
        matched_handle=matched,
        signature_valid=sig_ok,
        biometric_matched=biometric_matched,
    )


def attest_biometric_match(matcher_keypair, *, asserted_handle: bytes, challenge: bytes) -> bytes:
    """The trusted matcher (Secure Enclave on device, or an accountable live-recovery
    attestor) signs the one-to-one biometric-match verdict, bound to THIS challenge, so
    `verify_one_to_one` can verify it. Replaces the old forgeable `live_biometric_matches`
    bool with an unforgeable, matcher-key-rooted attestation (security review #9)."""
    from ..crypto.sign import sign as sig_sign

    return sig_sign(matcher_keypair, H(b"atlas/biometric-match-1to1", asserted_handle, challenge))
