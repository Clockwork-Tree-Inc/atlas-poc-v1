...
def _public_msg_inputs(msg_index: int) -> tuple[bytes, bytes, bytes]:
    ...
    epoch_round = msg_index.to_bytes(8, "big")
    return entropy_t, beacon_t, epoch_round
...
def within_epoch_step(chain_key: bytes, msg_index: int) -> bytes:
    ...
    entropy_t, beacon_t, epoch_round = _public_msg_inputs(msg_index)
    return derivation.ratchet(
        chain_key, entropy_t=entropy_t, beacon_t=beacon_t, epoch_round=epoch_round
    )
