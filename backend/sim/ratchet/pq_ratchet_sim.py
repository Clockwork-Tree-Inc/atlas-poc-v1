...
def ratchet_step(prev_key: bytes) -> bytes:
    ...
    Inputs to derivation.ratchet:
      ...
      epoch_round — a PUBLIC randomness-beacon round (not secret)
    ...
    epoch_round = (12345).to_bytes(8, "big")    # public drand round number
    return derivation.ratchet(
        prev_key, entropy_t=entropy_t, beacon_t=beacon_t, epoch_round=epoch_round
    )
...
def prop_2_forward_secrecy():
    ...
    # beacon_t/epoch_round; entropy_t is a hashed fresh secret).
    keys = [seed]
    for _ in range(6):
    ...
    fwd_a = derivation.ratchet(kt, entropy_t=entropy, beacon_t=beacon, epoch_round=drand)
    fwd_b = derivation.ratchet(kt, entropy_t=entropy, beacon_t=beacon, epoch_round=drand)
    ...
    one_step_forward = derivation.ratchet(
        kt, entropy_t=entropy, beacon_t=beacon, epoch_round=drand
    )
...
def prop_3_pq_safe_chain():
    ...
    # Expected: prev_key + entropy_t, beacon_t, epoch_round — all symmetric/public.
...
def prop_4_decoupling_device_key():
    ...
    coupled_a = derivation.derive_session_key_coupled(
        ...
        epoch_round=common["epoch_round"],
    ).key
