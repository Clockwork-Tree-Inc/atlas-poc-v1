...
def main() -> int:
    ...
    for i in range(3):
        r = timed_ratchet_step(d, ambient, pole=live_pole(), epoch_round=b"\x00" * 8, beacon=BEACON)
    ...
    r = timed_ratchet_step(d, dead, pole=live_pole(), epoch_round=b"\x00" * 8, beacon=BEACON)
    ...
    try:
        timed_ratchet_step(d, RingSignalSource(), pole=live_pole(), epoch_round=b"\x00" * 8, beacon=BEACON)
