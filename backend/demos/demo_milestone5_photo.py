...
import os
import pickle  # nosec B403 - demo-only transport of locally-generated, tunnel-sealed data (never attacker-controlled)
import sys
...
def live_pole(rnd):
    ...
    return g.state(sensor_digest=b"sensor", epoch_round=rnd.epoch_round())
...
def main() -> int:
    ...
    inp = EpochInputs(lk=b"L" * 32, epoch_key=os.urandom(32), epoch_round=rnd.epoch_round())
    ...
    label = b"component|" + rnd.epoch_round()
    ...
    registry.publish(inp.lk, rnd.epoch_round())
    ...
    # nosec B301 - unpickles only our own payload after it round-trips the sealed tunnel (open_message
    # authenticates + decrypts with tB); the input is locally generated, never attacker-controlled.
    received = pickle.loads(open_message(sealed, key=tB))  # nosec B301
