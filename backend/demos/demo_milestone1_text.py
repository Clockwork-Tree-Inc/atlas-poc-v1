...
def pole_for(stream_fn, epoch_round: bytes):
    g = LivenessGate()
    for _, (psl, psnl) in stream_fn(40):
        g.update(p_s_given_live=psl, p_s_given_not_live=psnl)
    return g.state(sensor_digest=b"sensor", epoch_round=epoch_round)
...
def main() -> int:
    ...
    def run_epoch(now: float, label: str):
        ...
        draw = qrng.fire(arrivals, rnd.epoch_round())                       # LK (private QRNG)
        ...
        epoch_draw = qrng.fire(ArrivalTiming([now, now + 0.3, now + 0.7]), rnd.epoch_round())
        inp = EpochInputs(lk=draw.randomness, epoch_key=epoch_draw.randomness, epoch_round=rnd.epoch_round())
        ...
        beacon_label = b"component|" + rnd.epoch_round()
    ...
    msg_key_1 = tA
    msg_key_2, secret_entropy = A.message_ratchet_step(msg_key_1, beacon_t=rnd.randomness, epoch_round=rnd.epoch_round())
    ...
    b_key_2 = ratchet(msg_key_1, entropy_t=secret_entropy, beacon_t=rnd.randomness, epoch_round=rnd.epoch_round())
    ...
    attacker_guess = ratchet(msg_key_1, entropy_t=b"\x00" * 32, beacon_t=rnd.randomness, epoch_round=rnd.epoch_round())
    try:
    ...
    epoch_component = b"component|" + rnd.epoch_round()
    ...
    live_pole = pole_for(live_stream, rnd.epoch_round())
    ...
    out = open_message(m3, key=tB, current_beacon_component=epoch_component,
                       attestation_provider=live_provider, expected_epoch_round=rnd.epoch_round())
    ...
    spoof_pole = pole_for(spoof_stream, rnd.epoch_round())
