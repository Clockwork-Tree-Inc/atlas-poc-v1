...
def build() -> dict:
    ...
    vectors["ratchet"] = [{
        ...
        "epoch_round": hx(fixed(8, 0)),
        "output": hx(ratchet(fixed(32, 3), entropy_t=b"entropy-t", beacon_t=b"beacon-t", epoch_round=fixed(8, 0))),
    ...
    sk = derive_session_key_decoupled(
        ...
        prev_key=fixed(32, 0), context_separator=CONTEXT_TUNNEL, epoch_round=fixed(8, 1))
    vectors["session_key_decoupled"] = [{
        ...
        "context_separator": hx(CONTEXT_TUNNEL), "epoch_round": hx(fixed(8, 1)),
    ...
    epoch_round8 = fixed(8, 0x07)
    enroll_secret = fixed(32, 0x61)
    epoch_key = fixed(32, 0x62)
    lk_secret = fixed(32, 0x63)
    unwrap_k = _unwrap_key(enroll_secret, epoch_round8)
    lk_k = _lk_key(epoch_key, epoch_round8)
    nonce = fixed(12, 0x00)
    ...
    assert unwrap_epoch_key(wrapped_epoch, presence_secret=enroll_secret, epoch_round=epoch_round8) == epoch_key
    assert unlock_lk(wrapped_lk, epoch_key=epoch_key, epoch_round=epoch_round8) == lk_secret
    vectors["presence_unwrap_chain"] = [{
        "epoch_round": hx(epoch_round8),
        "enrollment_secret": hx(enroll_secret), "epoch_key": hx(epoch_key), "lk": hx(lk_secret),
        "unwrap_key": hx(unwrap_k), "unwrap_key_info": ("atlas/epoch-unwrap|" + epoch_round8.hex()),
        "lk_key": hx(lk_k), "lk_key_info": ("atlas/lk-unlock|" + epoch_round8.hex()),
    ...
    vectors["live_provenance_binding"] = [{
        ...
        "content_hash": hx(content_hash_lb), "epoch_round": hx(epoch_round8),
        "authorship_handle": hx(handle_lb),
        "witness_seed": hx(_witness_seed(lk_lb, epoch_round8)),
        "session_commit": hx(sc),
        "attribution_core": hx(_attribution_core(content_hash_lb, epoch_round8, handle_lb, sc)),
    }]
    ...
    ctx = fixed(16, 0x5A)
    e0 = ForensicEvent(seq=0, prev_hash=b"\x00" * 32, epoch_round=epoch_round8,
                       decision=DecisionType.LOGIN, outcome=Outcome.ALLOW,
                       risk=RiskLevel.ROUTINE, context_handle=ctx, signal_digest=sd_a)
    e1 = ForensicEvent(seq=1, prev_hash=e0.event_hash(), epoch_round=epoch_round8,
    ...
    vectors["forensic_event_chain"] = [
        {"seq": 0, "prev_hash": hx(b"\x00" * 32), "epoch_round": hx(epoch_round8),
         "decision": "login", "outcome": "allow", "risk": 0,
         "context_handle": hx(ctx), "signal_digest": hx(sd_a), "output": hx(e0.event_hash())},
        {"seq": 1, "prev_hash": hx(e0.event_hash()), "epoch_round": hx(epoch_round8),
    ...
    vectors["global_anchor"] = [
        {"prev": hx(_GAL.GENESIS), "owner_id": hx(ga_owner_a), "root": hx(lg_root),
         "epoch_round": hx(fixed(8, 0x07)), "index": 0, "entry_hash": hx(ga_r1.entry_hash)},
        {"prev": hx(ga_r1.entry_hash), "owner_id": hx(ga_owner_b), "root": hx(_mk.empty_root()),
         "epoch_round": hx(fixed(8, 0x08)), "index": 1, "entry_hash": hx(ga_r2.entry_hash)},
    ]
