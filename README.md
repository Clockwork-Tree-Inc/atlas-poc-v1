...
Beta implementation of the Atlas protocol — a post-quantum, live-presence
...
**device-only** (needs a physical iPhone — Secure Enclave, wearable, camera — so it's
exercised on hardware, not in CI), and **not built**.

Both the Python core **and** the Swift `AtlasCore` reach **built + tested**:
**CI runs them on every push** — the backend suite (**788 tests**, on the pure-Python
default *and* the native-Ursa path) plus the Swift core/parity suite (**313 tests**) on
macOS. See [`.github/workflows/ci.yml`](.github/workflows/ci.yml).

**Maturity: beta — open for testing** (not "finished"). The built core is real, tested, and
cross-language-verified; it is now offered for others to run and attack, not just read. It stays
honest about what is and isn't finished: where a seam is a stub or a layer is designed-not-built, it
says so plainly below, and every built/tested/sim/device-gated/designed tag still means exactly what
it says. Because it is open source (AGPL-3.0), those gaps are **invitations to contribute**, not
hidden debt.

**Two axes, kept separate (read this before "liveness").** (1) The **security + identity axis** —
the sampled/phone-**ambient signal source is now confirmed functional**: it drives the engine
end-to-end, feeding entropy/timing into key derivation, the ratchet, and the identity/session
machinery. (2) The **signal-quality / anti-spoof axis** — proving a signal is *live* (ambient → ring
→ high-fidelity wearable). These are independent. The ambient source **makes no liveness claim on its
own**: it powers the engine; it does **not** prove anti-spoof. Never read "ambient confirmed
functional" as "liveness proven via ambient" — it is not.
...
| 3 | In-person enrolment ritual + co-motion wearable-lock | ✅ identity/secret construction tested | co-motion + biometric capture on device |
...
Notes:
- "**built + tested**" = passes in CI: 788 backend tests + the Swift `AtlasCore` suite (313 tests).
  M2's liveness is tested **in simulation** (synthetic streams), not a real wearable; M5's PAD
...
token is NOT live and is NOT offered in beta** — it goes live only at **full release**. To be
unambiguous: **no token is offered in beta; this is not an investment; the token launches only at
full release.**
...
**See a real behaviour in ~10 seconds** — a sealed message opens while a live human is present, then
*refuses to open* after a liveness break (by construction: no presence → the enclave withholds the
secret → the unwrap fails; there is no code path around it):
...
cd backend && pip install -e .        # pure-Python by default (Ursa BBS+ is optional)
atlas-demo-liveness                   # the headline demo above
atlas-demo-records                    # sealed medical/records lifecycle (grants, retention, break-glass)
```
...
```bash
cd backend
python -m pytest -q                   # the full test suite (940 tests)
python -m atlas.net.node_server       # run a blind relay node (→ http://127.0.0.1:8787)
python -m demos.demo_milestone1_text  # more demos in backend/demos/
